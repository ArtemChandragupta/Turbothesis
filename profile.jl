...
# ╔═╡ 5cc218a4-ab39-11f0-1a1e-35289b3cda35
begin
	using PlutoUI
	using LaTeXStrings
	using CairoMakie
...
function build_profile(α₁, β₁, α₂, β₂, c₁, w₁, c₂, w₂, l, ξ, R₁, R₂, Δ₁, Δ₂)
	# Нормализация углов
...
build_profile(36.51, 56.94, 84.32, 47.72, 0.6761, 0.4839, 0.3996, 0.5339, 2.0, 0.3, 0.05, 0.03, 10, 10)

# ╔═╡ 06ece98b-ecf6-4f23-b102-85b2821c91bc
function compute_distances(pline_old, xp, sline_new, xs)
    distances = Float64[]
    for x in xp
...
function build_shifted_profile(α₁, β₁, α₂, β₂, c₁, w₁, c₂, w₂, l, ξ, R₁, R₂, Δ₁, Δ₂, Δy)
    # Нормализация углов
    α₁, β₁ = α₁ - 90, β₁ - 90  # Углы у входной кромки
    α₂, β₂ = 90 - α₂, 90 - β₂  # Углы у выходной кромки
    ...
    # Точки для сплайнов профиля
    p1 = ( R₁ * cosd(90 + β₁ₚ)    ,  R₁ * sind(90 + β₁ₚ)    )
    p2 = ( R₂ * cosd(90 + β₂ₚ) + l,  R₂ * sind(90 + β₂ₚ) + ξ)
    s1 = (-R₁ * cosd(90 + β₁ₛ)    , -R₁ * sind(90 + β₁ₛ)    )
    s2 = (-R₂ * cosd(90 + β₂ₛ) + l, -R₂ * sind(90 + β₂ₛ) + ξ)
    
    # Построение сплайнов профиля
    cline = hermite_polynomial(0, 0 , tand(β₁ ), l, ξ , tand(β₂ ))
    pline = hermite_polynomial(p1..., tand(β₁ₚ), p2..., tand(β₂ₚ))
    sline = hermite_polynomial(s1..., tand(β₁ₛ), s2..., tand(β₂ₛ))
    
    # Точки для сплайнов сдвинутого профиля
    p1_shifted = (p1[1], p1[2] + Δy)
    p2_shifted = (p2[1], p2[2] + Δy)
    s1_shifted = (s1[1], s1[2] + Δy)
    s2_shifted = (s2[1], s2[2] + Δy)
    
    # Построение сплайнов сдвинутого профиля
	pline_new = hermite_polynomial(p1_shifted..., tand(β₁ₚ), p2_shifted..., tand(β₂ₚ))
    ...
    # Вычисление расстояний от каждой точки p до сдвинутой s
    distances = compute_distances(pline, xp, sline_new, xs)
	...
	with_theme(theme_latexfonts()) do
    	...
    	for (i, x) in enumerate(xp)
			...
			lines!(ax, [x, x_new_lower], [y_old_upper, y_new_lower], color=colors[i], linewidth=4)
    	end

		# Дуги скругления профиля
		arc!(ax, (0, 0), R₁, deg2rad(90 + β₁ₚ), deg2rad(360 - 90 + β₁ₛ), color=:black)
		arc!(ax, (l, ξ), R₂, deg2rad(90 + β₂ₚ), deg2rad(     -90 + β₂ₛ), color=:black)
    
	    # Дуги скругления сдвинутого профиля
		arc!(ax, (0, Δy), R₁, deg2rad(90 + β₁ₚ), deg2rad(360 - 90 + β₁ₛ), color=:black)
