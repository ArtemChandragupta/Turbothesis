...
begin
	...
	const CONST = (
		...
		σ⃰ₖₛ  = 0.96,  # Коэфф. потерь в камере сгорания
		σ⃰₄   = 0.985, # Коэфф. потерь в выходном устройстве
		
		ηₐ   = 0.91,  # Адиабатный КПД компрессора
		ηₚ   = 0.87,  # Политропный КПД турбины
		ηₘₜ  = 0.995, # Коэфф. мех. потерь в турбине
		ηₘₖ  = 0.995, # Коэфф. мех. потерь в компрессоре
		ηₖₛ  = 0.98,  # КПД камеры сгорания
...
function calc_comp(I, π⃰ₖ, 𝒞 = CONST, 𝒯 = TASK)
	...
	(; P⃰₁, T⃰₁, P⃰ₖ, P⃰₂, ρ₁, nₖ, ρ₂, D₁, Dᵥₜ₁, Dₘ₁, l₁, F₂, ν₂, l₂, uₙ₁, H⃰ₐ, H⃰ₖ, i, h₁, h₂, uₘ₁, Φ₁, h̄₁, otn, otm, P₀ᵍ, J, tb, tbem, u, cᵤ₁, cᵤ₂, c₁, α₁, T₁, wᵤ₁, w₁, Mʷ₁, β₁, Δcᶻ, Ccᶻ₂, c₂, α₂, wᵤ₂, w₂, β₂, ϵ, Φₙ, Mʷₘ)
end
