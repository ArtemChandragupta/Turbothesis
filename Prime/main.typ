...
#figure(
  [
    ...
    1-2 --- адиабатное сжатие в компрессоре, 2-3 --- изобарный повод теплоты в камере сгорания, 3-4 --- адиабатное расширение продуктов сгорания на лопатках газовой турбины, 4-1 --- изобарный отвод теплоты от продуктов сгорания в атмосферу
  ],
...
$ g_в &= ( Q_p^н dot eta_"кс" + h_т + L_0 dot c_p_в dot t_2^* - (L_0 + 1) dot (c_p_г)_(alpha=1) dot t_3^* ) / (c_p_в dot (t_3^* - t_2^*))= \ &= ( COQₙₚ dot COηₖₛ + COL0 dot COCpₙ dot Its2 - (COL0 + 1) dot COCpₐ dot Its3 )/(COCpₙ dot (Its3-Its3)) = Igₐᵢᵣ; $
...
Максимальный КПД установки достигается при максимальной температуре газа перед турбиной – $1743 К$. Жаростойкость материала лопаток турбины не позволяет выдерживать такую температуру, поэтому в качестве входной температуры на турбину выберем $AATs0 К$. Экстремум графика зависимости эффективного КПД ГТУ от степени повышения давления в компрессоре наблюдается при $pi_к^* = $ и $eta_e = $. Выбор такой степени сжатия неоправдан, т. к. при нём слишком низкие значения эффективной удельной работы и коэффициента полезной работы. Экстремум графика зависимости эффективной удельной работы ГТУ от степени повышения давления в компрессоре наблюдается при $pi_к^* = AAπsₖ $, значение эффективного КПД ГТУ при этом $eta_e = $. Коэффициент полезной работы ГТУ с увеличением  монотонно уменьшается, однако уменьшение  с целью увеличения  нецелесообразно, поскольку величина коэффициента полезной работы ГТУ увеличивается незначительно, при этом снижается величина эффективной удельной работы.
...
$ rho_2 = rho_1 (P_2^* / P_1^*)^(1/n) = Cρ1 (CPs2 / CPs1)^(1/Cnₖ)  = Cρ2; $

$ k_в / (k_в-1) dot eta_"ад"^* = n/(n-1) $

$ COkₙ / (COkₙ - 1) dot COηₐ = n/(n-1) => n = Cnₖ; $

Примем величины осевой составляющей абсолютных скоростей в сечениях 1-1 и 2-2 соответственно $C_(z_1) = COcᶻ1 "м/с" $ и $C_(z_2) = COcᶻ2 "м/с" $. Втулочное отношение выберем $nu_1 = COν1 $. Расход воздуха $G_"в" = IGₙ "кг/с" $.

Из уравнения расхода первой ступени выразим значение наружного диаметра на входе в компрессор:
$ G_в &= rho_1 dot pi/4 dot (D^2_Н_1 - D^2_"вт"_1) dot C_z_1 = \ &= Cρ1 dot pi/4 dot (CD1^2 - CDᵥₜ1^2) dot COcᶻ1 = IGₙ "кг/с"; $

Откуда,
$ D_Н_1 &= sqrt( (4 G_в)/(rho_1 dot pi dot (1-nu_1^2) dot C_z_1) ) = \  &= sqrt( (4 IGₙ) / (Cρ1 dot pi dot (1-COν1^2) dot COcᶻ1 ) ) = CD1 " м"; $
...
Для расчета частоты вращения необходимо задать окружную скорость на наружном диаметре первой ступени $u_н_1 = Cuₙ1 "м/с" $, тогда:
...
$ C_1 = sqrt(C^2_z_1 + C^2_u_1) = sqrt( COcᶻ1 + COcᶻ2 ) = Cc1 "м/с"; $
...
$ H_"ут" = k_N dot N_т/G_г = COkₙₜ dot TNₜ / TGᵧ = THᵤₜ; $
...
$ alpha_"кр"_2 = sqrt( (2 dot k_г)/(k_г+1) R_г T^*_(2 т) ) = sqrt( (2 dot COkᵧ)/(COkᵧ+1) dot CORᵧ dot TTs2ₜ ) = Taᵏʳ2; $
...
$ P_(2 т) = P_0^* ( T^*_(2t_T)/T_0^* )^( k_г/(k_г-1) ) = TPs0 (TTs2ₜₜ / AATs0)^(COkk1) = TP2ₜ; $
...
В результате теплового расчета была выявлена оптимальная температура перед турбиной $T_3^* = AATs0 " K"$. Были получены оптимальные параметры
