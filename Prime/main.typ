...
#block(breakable: false)[Температура воздуха перед компрессором:
$ T_1^* = T_Н = TATₙ "K;" $]
...
#block(breakable: false)[Давление газа за турбиной:
...
#block(breakable: false)[Относительное количество воздуха, содержащегося в продуктах сгорания за камерой сгорания:
...
По методическим указаниям @TURB произведем расчет параметров турбины. Из ранее полученных результатов мы получили основные значения для предварительного расчета турбины.
...
+ Рабочее тело --- газ со следующими характеристиками @Tutu:
  ...
  - Показатель изоэнтропы: $k_г = COkᵧ $;
  - Изобарная теплоемкость при заданной температуре и давлении перед турбиной: $С_p_г = COCpᵧ "Дж"/("кг" dot "K") $.
...
$ alpha_"кр"_2 = sqrt( 2 k_г/(k_г+1) R_г T^*_(2 т) ) = sqrt( 2 dot COkᵧ/(COkᵧ+1) dot CORᵧ dot TTs2ₜ ) = Taᵏʳ2 "м/с"; $
...
$ H_"ад.т." = H_"ут" + C^2_(2т) / 2 = THᵤₜ + Tc2ₜ^2 / 2 = THₐₜ "Дж/кг"; $

#block(breakable: false)[Изоэнтропийный перепад энтальпий на турбину:
...
#block(breakable: false)[Напряжения в корневом сечении рабочей лопатки:
...
#noind коэффициент запаса имеет значение в допустимых пределах $K_"пр">=1.5$, следовательно, условие прочности выполняется.

Далее следует выбрать средний диаметр. Его выбирают, ориентируясь на диаметральные габариты компрессора и камеры сгорания, и таким образом, чтобы окружная скорость на среднем диаметре не превышала $500 "м/с"$. Если она меньше $300 "м/с"$, то следует увеличить диаметр или частоту вращения ротора.
...
$ Y = sqrt( (sum u_2^2) / (2 H_"от")) = sqrt( (4 dot Tu2^2) / (2 dot THₒₜ)) = TY, $
