...
$ g_"охл"^с = 0.11 + 0.25 dot 10^(-4) dot (T_3^* - T_"ст") = $

Расход воздуха для охлаждения ротора:
$ g_"охл"^р = 0.08 + 0.22 dot 10^(-4) dot (T_3^* - T_"ст") = $

Общий расход воздуха для охлаждения турбины:
$ g_"охл" = sigma_"уг" dot (g_"охл"^с + g_"охл"^р) = $

Относительный расход охлаждающего воздуха по отнoшению к расходу воздуха через компрессор:
$ g'_"охл" = ((1+g_т) dot g_"охл") / (1+(1+g_т) dot g_"охл") = $

Расход топлива:
$ G_т = g_т dot (1-g'_"охл") dot G_в = $

Коэффициент располагаемой мощности:
$ Omega_"рас" = H_"от"^* dot G_в/G_т = $

Удельная эффективная работа ГТУ:
$ H_e = (1+g_т) dot (1-g'_"охл") dot H_т dot eta_"мт" - H_к / eta_"мк"= $

Коэффициент полезной мощности:
$ Omega_"пол" = H_e^* dot G_в / G_т = $
...
$ H_к^* = H^*_"ад. пр. ч." / eta^*_"ад" = $

Выберем средний теоретический напор $h_"ср" = $.
...
+ Рабочее тело --- газ со следующими характеристиками @Tutu:
  ...
  - Показатель изоэнтропы: $k_г = $;
  - Изобарная теплоёмкость при заданной температуре и давлении перед турбиной: $c_p_г = $
