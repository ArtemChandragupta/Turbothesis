...
Для данного расчета примем $d_(2 т) = Td2ₘ "м."$ Тогда окружная скорость на среднем диаметре рабочего колеса последней ступени:
$ u_2 = (pi dot d_(2 т) dot n)/ 60 = (pi dot Td2ₘ dot TAn)/60 = Tu2 "м/с"; $

Высота лопаток последней ступени:
$ l_2 = F_(2 т)/(pi dot d_(2 т)) = TF2ₜ / (pi dot Td2ₘ) = Tl2 "м," $

#noind в результате чего параметр $d_(2 т) / l_2$:
$ d_(2 т) / l_2 = Td2ₘ / Tl2 = Tkₘ; $

Примем число ступеней турбины $m = 4$. Тогда характерный напорный параметр $Y$ равен:
$ Y = sqrt( (sum u_2^2) / (2 H_"от")) = sqrt( (4 dot Tu2^2) / (2 dot THₒₜ)) = COY, $
...
В результате теплового расчета была выявлена оптимальная температура перед турбиной $T_3^* = AATs0 "K."$ Были получены оптимальные параметры \ $eta_e = 0.35$ и $pi_k^* = AAπsₖ$.
...
Подводя итог расчета турбинной части, была получена 4-ступенчатая турбина. Высота последней лопатки $l_2 = #calc.round(RawTl2 * 1000) "мм."$ Средний диаметр рабочих лопаток $d_(2т) = Td2ₘ "м."$
