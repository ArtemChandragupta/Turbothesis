...
+ Безразмерная скорость потока за турбиной
...
$ sigma_p = 0.89 dot 10^(-5) dot n^2 dot F_(2т) = 0.89 dot 10^(-5) dot  $

Выберем материал для лопаток --- сталь ЭИ437Б @STEL, для которой предел длительной прочности $[sigma_500]= "МПа" $ и находим коэффициент запаса прочности:
...
Для данного расчета примем $d_(2 т) = 1.4 м$. Тогда окружная скорость на среднем диаметре рабочего колеса последней ступени:
$ u_2 = (pi dot d_(2 т) dot n)/ 60 = $

Высота лопаток последней ступени:
$ l_2 = F_(2 т)/(pi dot d_(2 т)) = $

в результате чего параметр $d_(2 т) / l_2$:
$ d_(2 т) / l_2 = $

Примем число ступеней турбины $m = 4$. Тогда характерный напорный параметр $Y$ равен:
$ Y = sqrt( (sum u_2^2) / (2 H_"от")) = $
...
Подводя итог расчета турбинной части, была получена 4-ступенчатая турбина. Высота последней лопатки $l_2 = $. Средний диаметр рабочих лопаток $d_(2т)=$
