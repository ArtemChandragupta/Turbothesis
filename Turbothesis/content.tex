...
$R_{\text{г}}= \RN \ \dfrac{\text{кДж}}{\text{кг}\cdot \text{К}}$ - индивидуальная газовая постоянная;

$c_{p_{\text{г}}}=\CPg \ \dfrac{\text{кДж}}{\text{кг}\cdot \text{К}}$ - удельная изобарная теплоёмкость газа;
...
где $Q_{\text{p}}^{\text{н}}=\Qnp \ \dfrac{\text{кДж}}{\text{К}}$ - низшая теплота сгорания топлива;

$h_{\text{т}}=\htop$ - Энтальпия топлива (пренебрежимо мала);

$(c_{p_{\text{г}}})_{\alpha=1}=\Cpao\ \dfrac{\text{кДж}}{\text{К}}$ - Удельная теплоемкость газа при $\alpha=1$;

$L_{0}=15 \ \dfrac{\text{кг}_{\text{возд}}}{\text{кг}_{\text{топ}}}$ - Стехиометрический коэффициент.

\

Коэффициент избытка воздуха в камере сгорания:
$$\alpha= \dfrac{L_{0}+g_{\text{в}}}{L_{0}}=\dfrac{\Lo+1}{\Lo}=\aaa;$$
...
Примем величины осевой составляющей абсолютных скоростей в сечениях 1-1 и 2-2 соответственно $C_{z_1}=\Czone \  \dfrac{\text{м}}{\text{с}}$ и $C_{z_2}=\Cztwo \  \dfrac{\text{м}}{\text{с}}$. Втулочное отношение выберем $V_{1}=\dfrac{D_{\text{вт}_{1}}}{D_{\text{н}_{1}}}=\Vone$. Расход воздуха $G_{\text{в}}=\GNN \ \dfrac{\text{кг}}{\text{с}}$.
...
\inputpgf{Images/dgf}{COMP-ome-approx.pgf}{График зависимости $\left(\dfrac{\bar{h}_{1}}{\varphi} \right)_{\tfrac{b}{t}=1}$ от $\dfrac{\Omega}{\varphi}$}
