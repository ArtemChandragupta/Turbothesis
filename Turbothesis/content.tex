...
\begin{equation} \label{}
  \Omega_{\text{пол}}=H_{e}^* \cdot \dfrac{{G_{\text{в}}}}{G_{\text{т}}}=\He \cdot \dfrac{\GN}{\Gt}=\Omepol\ \dfrac{\text{кДж}}{\text{К}};
\end{equation}
...
\begin{enumerate} 
  \item Полное давление и полная температура на входе в турбину:
	...
	\end{equation}
	\begin{eqexpl}
		\item{$P_{\text{к}}$} давление на выходе из компрессора (найдено в Главе 3,\\ $P_{\text{к}}^*=\Pk \ \text{МПа}$);
...
\begin{equation} \label{}
	H_{\text{от}}=\dfrac{H_{\text{ад.т}}}{\eta_{\text{ад.т}}}=\dfrac{\Hadm}{0.91}=\Hott \ \dfrac{\text{кДж}}{\text{кг}};
\end{equation}

Температура в потоке за турбиной при изоэнтропийном процессе расширения:
\begin{equation} \label{}
	T_{2t_{T}}^*=T_{0}- \dfrac{H_{\text{от}}}{c_{p_{{\text{г}}}}}=\To -  \dfrac{\Hott \cdot 10^3}{\Cpg}=\Ttwott \ \text{К};
\end{equation}
