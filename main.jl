...
begin
	function calc_comp(Å, 𝒞 = CONST, 𝒯 = TASK)
		P⃰₁   = 𝒞.σ⃰ᵢₙ * 𝒯.Pₙ
		T⃰₁   = 𝒯.Tₙ
		P⃰ₖ   = 𝒯.Pₙ * Å.π⃰ₖ
		P⃰₂   = P⃰ₖ / 𝒞.σ⃰ₒᵤₜ
...
begin
	function calc_comb(Å, comp, 𝒞 = CONST, 𝒯 = TASK)
		Dᵢₙ  = 1.69
		Fᵢₙ  = 0.22
		hᵢₙ  = 0.086
		Dₒᵤₜ = 1.21
		...
		Tₖ = comp.T⃰₁ * (Å.π⃰ₖ)^( (comp.nₖ - 1)/comp.nₖ )
		P⃰₂ = comp.P⃰ₖ / 10^6
		
		Fₘ   = Fₒᵤₜ * X₁
		μFₒₘ = 1.6 * 10^(-5) * (Å.Gₙ * Tₖ^0.5) / (P⃰₂ * (1 - 0.83 * X₂^2)^2.5 * X₂ )
		Dᵐₜᵤ = (Dᵢₙ + Dₒᵤₜ)/2
		hᵐₜᵤ = Fₘ / (π * Dᵐₜᵤ)
		Dᵗᵒᵖₜᵤ = Dᵐₜᵤ + hᵐₜᵤ
		Dᵇᵒᵗₜᵤ = Dᵐₜᵤ - hᵐₜᵤ
		...
		Lᵧ   = X₃ * hᵐₘ
		Fₖₖ  = X₄ * Fᵢₙ
		Dᵗᵒᵖ = √(Dᵗᵒᵖₜᵤ^2 + 2/π * Fₖₖ)
		Dᵇᵒᵗ = √(Dᵇᵒᵗₜᵤ^2 + 2/π * Fₖₖ)
		Lᵦₐ = 0.8 * hᵐₜᵤ
		Lₜᵤ = Lᵦₐ + Lᵧ
		nᵈ₁ = 5αd^(-0.38)
		hᵈ₁ = nᵈ₁ * hᵢₙ
		...
		Vₜᵤ = 0.8 * Fₘ * Lₜᵤ
		Vᵦₐ = Fₘ * Lᵦₐ
		Sˡ  = (Dᵗᵒᵖₜᵤ + Dᵇᵒᵗₜᵤ) * π * Lₜᵤ
		Kₛ  = 0.542 * Sˡ + 0.687
		Sₜᵤ = Kₛ * Sˡ

		k₁ = (0.23(1 - Fᵢₙ/Fₖₖ) * π * Dᵢₙ * ( (hᵈ₁ + hᵢₙ) * Lᵈ₁ + Δ * (Dᵗᵒᵖ-Dᵇᵒᵗ) ) * hᵢₙ^0.38 ) / (2 * Fᵢₙ * (Δ + Lᵈ₁)^1.38 )
		k₂ = Fₘ / μFₒₘ
		k₃ = Fₖₖ / μFₒₘ
		tf = π * Dᵐₜᵤ / (Nᶠ * hᵐₜᵤ)
		Tω = Tₖ + 30.6 / (1 + 0.8α * L₀) * ( -0.11 * (Sₜᵤ/Vₜᵤ)^2 + 19.3 * (Sₜᵤ/Vₜᵤ) )

		# EINOX = 710.63 * P⃰₂^1.5 * Vᵦₐ / Å.Gₙ * Å.T⃰₀/Tₖ * exp(Tₖ/288) * ( 1 - 0.181Fₘ/μFₒₘ + 0.0124 * (Fₘ/μFₒₘ)^2) * (1 + 0.277 * exp(-0.01 * Nᶠ / Fₘ)) * exp(-0.08W̄ - 0.492μF̄₃)
		
		# KₖCO = exp(1.1 * 10^(-5) * (Nᶠ/Fₘ)^(-2) - 1.02 * 10^(-2) * Nᶠ/Fₘ) * (1 + 8.11μF̄₃) * (1 + 0.118 * Fₘ/μFₒₘ) * (1 - 1.09W̄ + 0.463W̄^2) * (1 + 2.105 * de - 1.59 * de^2) * (0.924 + 0.123 * μF̄₃/μFₒₘ)
		# EICO = 191.78 * P⃰₂^(-1.8) * Å.Gₙ/(Vₜᵤ * Å.T⃰₀) * exp(-Tₖ/288) * KₖCO
		
		# KₖHC = exp(4 * 10^(-6) * (Nᶠ/Fₘ)^(-2) - 2.85 * 10^(-3) * Nᶠ/Fₘ) * (1 - 8.8μF̄₃ + 24.45 * μF̄₃^2) * (1 + 0.108 * Fₘ/μFₒₘ) * (1 - 0.901W̄ + 0.295W̄^2) * (1 + 7.51 * de - 6.77 * de^2) * (0.925 + 0.279 * μF̄₃/μFₒₘ)
		# EIHC = 43.35 * P⃰₂^(-1.8) * Å.Gₙ/(Vₜᵤ * Å.T⃰₀) * exp(-Tₖ/288) * KₖHC
		...
		MNOX = EINOX * 1.25 * 1000 / ((1+ α*L₀) * 2.05 )
		MCO  = EICO  * 1.25 * 1000 / ((1+ α*L₀) * 1.25 )
		MHC  = EIHC  * 1.25 * 1000 / ((1+ α*L₀) * 0.715)

		# EINOX = 943.63 * P⃰₂^1.5 * Vᵦₐ / Å.Gₙ * Å.T⃰₀/Tₖ * exp(Tₖ/288) * ( 1 - 0.1Fₘ/μFₒₘ + 0.005 * (Fₘ/μFₒₘ)^2) * (1 + 0.348 * exp(-0.01 * Nᶠ / Fₘ)) * exp(-0.204W̄ - 0.743μF̄₃)
		
		# KₖCO = (1 - 2.216μF̄₃ + 6.77 * μF̄₃^2) * (1- 0.104(Fₘ/μFₒₘ) + 0.007 * (Fₘ/μFₒₘ)^2 ) * (1 - 0.15W̄ + 0.456W̄^2) * (1 + 1.424 * exp(2))
		# EICO = 48.24 * P⃰₂^(-1.8) * Å.Gₙ/(Vₜᵤ * Å.T⃰₀) * exp(-Tₖ/288) * KₖCO * exp(4.75 * 10^(-6) * (Nᶠ/Fₘ)^2 - 2.86 * 10^(-3) * Nᶠ/Fₘ)
		
		# KₖHC = (1 - 3.33μF̄₃ + 5 * μF̄₃^2) * (1 - 0.153 * Fₘ/μFₒₘ + 0)
		# EIHC = 3.47 * P⃰₂^(-1.8) * Å.Gₙ/(Vₜᵤ * Å.T⃰₀) * exp(-Tₖ/288) * KₖHC * exp(1.7 * 10^(-5) * (Nᶠ/Fₘ)^2 - 1.545 * 10^(-2) * Nᶠ/Fₘ)

		EIBP = 2.51 * 10^(-5) * EIHC
		η = 1 - 10^(-3) * (EIHC + 0.232 * EICO)
		Θₘₐₓ = 1.2 * ( 0.0144 * (Fₘ / μFₒₘ)^2 - 0.178 * (Fₘ / μFₒₘ) + 1 ) * exp(0.377 * hᵐₘ / Lᵧ) / ( (Fₘ/μFₒₘ)^1.11 * (1 - 1.19 * exp(-Fₖₖ / μFₒₘ)) )
		Θʳₘₐₓ = 0.275Θₘₐₓ
		ξ₀  = 0.974 * (1 + 0.008 * (Fₘ / μFₒₘ)) / (1 - 0.68 * exp(-Fₖₖ / μFₒₘ))
		ξd  = (1 - Fᵢₙ / (1.1Fₖₖ))^2
		ξₖₛ = ξd + ξ₀ * (Fₖₖ / μFₒₘ)^2 * (Fᵢₙ/Fₖₖ)^2 * (1 - δₛᵤₘ)^2
		λₖ  = 1.6 * Å.Gₙ * √Tₖ / (P⃰₂ * 10^5 * Fᵢₙ)
		δₖₛ = 1 - 0.583 * ξₖₛ * (1 - 0.167 * λₖ^2) * λₖ^2
		
		(; Dᵢₙ, Fᵢₙ, hᵢₙ, Dₒᵤₜ, Fₒᵤₜ, hₒᵤₜ, μF̄₃, W̄, X₁, X₂, X₃, X₄, α, L₀, δₛᵤₘ, αd, de, P⃰₂, Tₖ, Fₘ, μFₒₘ, Dᵐₜᵤ, hᵐₜᵤ, Dᵗᵒᵖₜᵤ, Dᵇᵒᵗₜᵤ, hᵐₘ, Lᵧ, Fₖₖ, Dᵗᵒᵖ, Dᵇᵒᵗ, Lᵦₐ, Lₜᵤ, nᵈ₁, hᵈ₁, Lᵈ₁, Δ, Lᶜᵒᵐᵇ, Nᶠ, Nᶠ₁, Vₜᵤ, Vᵦₐ, Sˡ, Kₛ, Sₜᵤ, k₁, k₂, k₃, tf, Tω, EINOX, KₖCO, EICO, KₖHC, EIHC, EIBP, MNOX, MCO, MHC, η, Θₘₐₓ, Θʳₘₐₓ, ξ₀, ξd, ξₖₛ, λₖ, δₖₛ)
	end
	
	md"λ Расчет камеры сгорания"
end

# ╔═╡ 40561c16-193e-4349-bc16-a7d9ceb55f62
begin
	function calc_turb(C, Å, 𝒞 = CONST, 𝒯 = TASK)
		T⃰₀   = Å.T⃰₀
		P⃰₀   = 𝒞.σ⃰ₖₛ * C.P⃰ₖ
		Nₖ   = C.H⃰ₖ * Å.Gₙ
		Nₜ   = 𝒯.N + Nₖ
		...
		(; T⃰₀, P⃰₀, Nₖ, Gᵧ, Nₜ, Hᵤₜ, ΔT⃰ₜ, T⃰₂ₜ, aᵏʳ₂, c₂ₜ, Hₐₜ, H₀ₜ, T⃰₂ₜₜ, P₂ₜ, T₂T, ρ₂ₜ, F₂ₜ, u₂, d₂ₘ, l₂, kₘ)
	end
...
begin
	function build_geometry(T, 𝒞 = CONST, 𝒯 = TASK)

		function yᵧ(xₜ, 𝒯 = TASK)
    		direction = [cosd(𝒯.γ), sind(𝒯.γ)]
    
    		t = (xₜ - 0) / direction[1]
    
    		yᵣₑₛ = T.l₂ + t * direction[2]
		end
		...
		function intᵧ(α, p, T = T, 𝒯 = TASK)
			v1 = [cosd( α ), sind( α )]
			v2 = [cosd(𝒯.γ), sind(𝒯.γ)]
			A  = [v1 -v2]
			t  = A \ ([0,T.l₂] - p)
			p + t[1] * v1
		end
		...
		aw = 2
		w₁ = [0.6, 0.42, 0.34, 0.32]
		β₁ = 3
		β₂ = 3
	
		ll₁, xl₁, xl¹, ll₂, xl₂, xl² = [zeros(2𝒞.m) for _ in 1:6]
		bl₁, bl₂ = [zeros(𝒞.m) for _ in 1:2]
		...
		bl₂[𝒞.m] = ll₂[2𝒞.m] / 4.4
		bl₂[𝒞.m-1] = 7/8 * bl₂[𝒞.m]
		bl₂[𝒞.m-2] = 7/8 * bl₂[𝒞.m-1]
		bl₂[𝒞.m-3] = 1.09 * bl₂[𝒞.m-2]

		for n in 𝒞.m:-1:2
			if n != 𝒞.m
				xl₂[2n] = xl₁[2n+1] - bl₁[n+1] / aw
				ll₂[2n] = yᵧ(xl₂[2n])
			end

			xl²[2n  ] = intₖ(90+β₂, [xl₂[2n], ll₂[2n]] )[1]
			xl²[2n-1] = xl²[2n] - bl₂[n]

			ll₂[2n-1] = intᵧ(90-β₂, [xl²[2n-1], 0] )[2]
			xl₂[2n-1] = intᵧ(90-β₂, [xl²[2n-1], 0] )[1]

			xl₁[2n] = xl²[2n-1] - bl₂[n] / aw
			ll₁[2n] = intᵧ(90-β₁, [xl₁[2n], 0] )[2]
		
			xl¹[2n] = intᵧ(90-β₁, [xl₁[2n], 0] )[1]
			bl₁[n] = ll₁[2n] * w₁[n]
			xl¹[2n-1] = xl¹[2n] - bl₁[n]
		
			ll₁[2n-1] = yᵧ(xl¹[2n-1])
			xl₁[2n-1] = intₖ(90+β₁, [xl¹[2n-1], ll₁[2n-1] ] )[1]
		end

		xl₂[2] = xl₁[3] - bl₁[2] / aw
		ll₂[2] = yᵧ(xl₂[2])

		xl²[2] = intₖ(90+β₂, [xl₂[2], ll₂[2]] )[1]
		xl²[1] = xl²[2] - bl₂[1]

		ll₂[1] = ll₂[2]
		xl₂[1] = intᵧ(90-β₂, [xl²[1], 0] )[1]

		xl₁[2] = xl²[1] - bl₂[1] / aw * 0.77
		ll₁[2] = ll₂[2]
...
begin
	function stage_params(T, Φ, Ψ, l̄, 𝒞 = CONST, 𝒯 = TASK)
		l₁  = Tuple(l̄.ll₁[2:2:end]) # Высота задней   кромки сл
		l₂  = Tuple(l̄.ll₂[2:2:end]) # Высота задней   кромки рл
		l₂₁ = Tuple(l̄.ll₂[1:2:end]) # Высота передней кромки рл
		b₁  = l̄.bl₁ # Ширина сл
		b₂  = l̄.bl₂ # Ширина рл
...
begin
	function calc_stages(G, T, Params, 𝒯 = TASK)
	    ...
	    for param in Params
    	    stage = calc_stage(p⃰_prev, T⃰_prev, G, param)
        	push!(S, stage)
        	p⃰_prev, T⃰_prev = stage.p⃰₂, stage.T⃰₂
    	end

	    H = 1.03 * sum(stage.Hᵤ for stage in S)
    	return (tuple(S...), H)
	end
...
begin
	...
	function calc_swirl(№, 𝒫, w₂u_R1, 𝓜, ɤ, 𝒞 = CONST, 𝒯 = TASK)
		...
		u₂   = 2π * r  * 𝒯.n / 60
		w₁u  = c₁u - u₁
		β₁   = w₁u > 0 ? atand(c₁z / w₁u) : 180 + atand(c₁z / w₁u)
		w₁   = √(c₁z^2 + w₁u^2 + c₁r^2)
		...
		c₂r  =  c₂z * tand(γ )
		c₂   = √(c₂z^2 + c₂u^2 + c₂r^2)
		α⃰₂   = c₂u > 0 ? 180 - atand(c₂z / c₂u) : - atand(c₂z / c₂u)
		w₂   = c₂z / sind(β⃰₂)
		T₁   = 𝓜.T⃰₀ - c₁^2 / 2𝒞.Cpᵧ
...
begin
	function find_Gₒₚₜ(T, Φ, Ψ, l̄)
	
		G➞ = range(T.Gᵧ - 20, T.Gᵧ + 20, length = 500)
		T➞ = map(G -> calc_G(G, T), G➞)
		
		Params➞ = [stage_params(Tᵢ, Φ, Ψ, l̄) for Tᵢ in T➞]
	    𝒮  = calc_stages.(G➞, T➞, Params➞)
	    𝒮₄ = [s[1][end] for s in 𝒮 ]
		ᾱ₂ = [s.α⃰₂ for s in 𝒮₄]
		H➞ = [s[end] for s in 𝒮 ]

		idx = argmin(abs.(ᾱ₂ .- 90))
		Gₒₚₜ = G➞[idx]
		Hᵢ   = H➞[idx]
...
begin
	function find_GΦΨ(T, Φ➞, Ψ➞, l̄)
		results = []
		good_results = []
	
		for Φᵢ in Φ➞
			for Ψᵢ in Ψ➞
				(Gᵢ, Hᵢ) = find_Gₒₚₜ(T, Φᵢ, Ψᵢ, l̄)
				if abs(Gᵢ - T.Gᵧ) < 0.1
...
begin
	...
	((Gₒₚₜ, Φ, Ψ), Ḡ) = find_GΦΨ(T, Φ➞, Ψ➞, l̄)
	P = stage_params(T, Φ, Ψ, l̄)
		...
		with_theme(theme_latexfonts()) do
			...
			for i in 1:Int(length(l̄.ll₁)/2)
				poly!(ax, color = color1, Point2f[
					(1000l̄.xl²[2i  ], 0              ),
					(1000l̄.xl₂[2i  ], 1000l̄.ll₂[2i]  ),
					(1000l̄.xl₂[2i-1], 1000l̄.ll₂[2i-1]),
					(1000l̄.xl²[2i-1], 0              )
				])
			end
...
begin
	function calc_con(ϕ₁, ϕ₂, r, b, c, type)
		c̄    = c/b
		t̄ₒₚₜ = type == 2 ?
			0.55(1-c̄) * (sind(ϕ₁+90)/sind(90-ϕ₂) * 180/(ϕ₂-ϕ₁) )^(1/3) :
			0.55(1-c̄) * (sind(-ϕ₁+90)/sind(90+ϕ₂) * 180/(180-ϕ₂+ϕ₁) )^(1/3)
		tₒₚₜ  = t̄ₒₚₜ * b
		Lᵒ    = 2π * r
		z     = round(Lᵒ / tₒₚₜ)
		t     = Lᵒ / z
...
begin
	function centroid(x1, y1, x2, y2, θ₁ₚ, θ₁ₛ, θ₂ₚ, θ₂ₛ, l, ξ, R₁, R₂)
    	total_area  = 0.0
	    weighted_cx = 0.0
    	weighted_cy = 0.0

		function circular_centroid(R, θ₁, θ₂, cx, cy)
			θ = - θ₁ + θ₂
		    A = abs((R^2 / 2) * (θ - sin(θ)))
		    d = (4R * sin(θ/2)^3) / (3(θ - sin(θ)))
	    	bisector_angle = (θ₁ + θ₂)/2
		...
		cx₁, cy₁, A₁ = circular_centroid(R₁, θ₁ₚ, θ₁ₛ, 0, 0)
		cx₂, cy₂, A₂ = circular_centroid(R₂, θ₂ₚ, θ₂ₛ, l, ξ)

	    (( (weighted_cx + A₁*cx₁ + A₂*cx₂) / (total_area + A₁ + A₂),
		   (weighted_cy + A₁*cy₁ + A₂*cy₂) / (total_area + A₂ + A₁) ),
		 [cx₁, cy₁, A₁], [cx₂, cy₂, A₂]
		)
	end
...
begin
	function profile_build(S, R, type, n, R₁, R₂, Δ₁, Δ₂, l̄, Pr)
		# Углы средней линии
		ϕ₁ = type == 2 ? R[n].β₁ - 90 : 90 - S[3].α⃰₂
		ϕ₂ = type == 2 ? 90 - R[n].β⃰₂ : R[n].α₁ - 90
		...
		if type == 2 
			r = R[n].r
			l = R[n].b - 2tand(l̄.β₁) * (r - R[1].r)
			ξ = l * (R[n].w₁u+R[n].w₂u) / (R[n].w₁u/tand(ϕ₁) + R[n].w₂u/tand(ϕ₂))
		elseif type == 1
			r = R[1].r + l̄.ll₁[end] * (n-1)/4
			l = l̄.bl₁[end] - 2tand(l̄.β₁) * (l̄.ll₁[end] * ((5 - n)/4) )
			ξ = l * (S[3].c₂u+R[n].c₁u) / (S[3].c₂u/tand(ϕ₁) + R[n].c₁u/tand(ϕ₂))
		end
		...
		# Точки для сплайнов
    	t1 = (-R₁ * sind(ϕ₁ᵗ)    ,  R₁ * cosd(ϕ₁ᵗ)    )
    	t2 = (-R₂ * sind(ϕ₂ᵗ) + l,  R₂ * cosd(ϕ₂ᵗ) + ξ)
    	b1 = ( R₁ * sind(ϕ₁ᵇ)    , -R₁ * cosd(ϕ₁ᵇ)    )
    	b2 = ( R₂ * sind(ϕ₂ᵇ) + l, -R₂ * cosd(ϕ₂ᵇ) + ξ)

	    # Построение сплайнов оригинального профиля
		cline = hermite_polynomial(0, 0 , tand(ϕ₁ ), l, ξ , tand(ϕ₂ ))
    	tline = hermite_polynomial(t1..., tand(ϕ₁ᵗ), t2..., tand(ϕ₂ᵗ))
    	bline = hermite_polynomial(b1..., tand(ϕ₁ᵇ), b2..., tand(ϕ₂ᵇ))
    	...
    	cntr, ctr1, ctr2 = centroid(xt,yt,xb,yb,
									deg2rad(90 + ϕ₁ᵗ), deg2rad(360 - 90 + ϕ₁ᵇ),
									deg2rad(90 + ϕ₂ᵗ), deg2rad(    - 90 + ϕ₂ᵇ),
									l,ξ, R₁,R₂)

		# Вычисление наибольшей толщины профиля и положения этого сечения
		cₘₐₓ = thickness(xt, yt, xb, yb)

		# Вычисление оптимального числа лопаток
		scope = type == 2 ? 1 : 5
		if n == scope
			Z = calc_con(ϕ₁, ϕ₂, r, b, cₘₐₓ.cₘ, type)
		else
			z = Pr.Z.z
			t = 2π * r / z
			Z = (; z, t )
	...
	function profile_show(Pr)
		with_theme(theme_latexfonts()) do
			...
			diam_points_2 = [
				(Pr.l + Pr.R₂ * cos(α)     , Pr.ξ + Pr.R₂ * sin(α)),
				(Pr.l - Pr.R₂ * cos(α)     , Pr.ξ - Pr.R₂ * sin(α)),
				(Pr.l - mod * (Pr.R₂ * cos(α) + dl), 
				 Pr.ξ - mod * (Pr.R₂ * sin(α) + dl * tan(α))),
        	]
			if Pr.type == 2
				lines!(ax, color = :gray, linewidth = 1,
...
begin
	function profile_combined(Pr)
		...
		with_theme(theme_latexfonts()) do
			...
			# Расстояния между линиями профилей
			ds = distance(Pr, Δ)
			...
			elseif Pr.type == 1
				for i in 1:length(Pr.xt)
					lines!(ax, color = colors[i], linewidth=4,
						   [Pr.xb[i], Pr.xt[ds[i].j]    ],
						   [Pr.yb[i] + Δ, Pr.yt[ds[i].j]],
					  	)
				end
			end

			# Отображение максимальной толщины и центроида
			arc!(ax, color = :gray, linewidth = 1, [Pr.cₘₐₓ.pₘ[1], Pr.cₘₐₓ.pₘ[2]+ Δ], Pr.cₘₐₓ.cₘ, 0, 2π)
			scatter!(ax, Pr.cntr, color=:gray, markersize=10)

			α  = atan(Pr.ξ / Pr.l) + pi/2
			dl = 0.4Pr.Z.t * cos(α)
    	    diam_points = [
				(Pr.cₘₐₓ.pₘ[1]+Pr.cₘₐₓ.cₘ*cos(α), Pr.cₘₐₓ.pₘ[2]+Pr.cₘₐₓ.cₘ*sin(α)+Δ),
				(Pr.cₘₐₓ.pₘ[1]-Pr.cₘₐₓ.cₘ*cos(α), Pr.cₘₐₓ.pₘ[2]-Pr.cₘₐₓ.cₘ*sin(α)+Δ),
				(
					Pr.cₘₐₓ.pₘ[1]+mod*(-Pr.cₘₐₓ.cₘ*cos(α) - 1.1 * dl),
					Pr.cₘₐₓ.pₘ[2]+mod*(-Pr.cₘₐₓ.cₘ*sin(α) - 1.1 * dl * tan(α)) +Δ
				)
        	]
			...
			# Контуры окружностей
			diam_points_1 = [
				(0 + Pr.R₁ * cos(α) * mod, 0 + Pr.R₁ * sin(α) * mod),
				(0 - Pr.R₁ * cos(α) * mod, 0 - Pr.R₁ * sin(α) * mod),
				(0 + Pr.R₁ * cos(α) * mod + dl * mod, 0 + Pr.R₁ * sin(α) * mod + dl * tan(α) * mod)
        	]
			lines!(ax, color = :gray, linewidth = 1,
				   [diam_points_1[2], diam_points_1[3]]
				  )
			...
			# Хорда
			lines!(ax, color = :gray, linewidth = 1, linestyle = :dash, [0, Pr.l], [0, Pr.ξ])
			bracket!(ax, color = :gray, linewidth = 1, 0, 0, Pr.l, Pr.ξ, orientation = dir, text = "b = $(round(Int, Pr.b * 10^3)) mm" )
		    ...
		    # Профиль лопатки
			lines!(ax, Pr.xc, Pr.yc .+ Δ, color = :gray , linewidth = 1)
    		lines!(ax, Pr.xt, Pr.yt .+ Δ, color = :black, linewidth = 2)
    		lines!(ax, Pr.xb, Pr.yb .+ Δ, color = :black, linewidth = 2)
	...
	md"👁 Отображение комбинированное"
end
	...
	function profiles_show(Pr1, Pr2, Pr3, Pr4, Pr5)
    	...
    	with_theme(theme_latexfonts()) do
	        ...
	        for i in 1:5
				...
				angles1 = range(deg2rad(90 + Pr[i].ϕ₁ᵗ), deg2rad(360 - 90 + Pr[i].ϕ₁ᵇ), length=n_points)
				arc1_points = [Point2f(arc_center1[1] + Pr[i].R₁ * cos(θ), arc_center1[2] + Pr[i].R₁ * sin(θ)) for θ in angles1]
            
	            # Вторая дуга (выходная кромка)
				angles2 = range(deg2rad(90 + Pr[i].ϕ₂ᵗ), deg2rad(-90 + Pr[i].ϕ₂ᵇ), length=n_points)
...
begin
	profile_combined(Pr1)
	profile_combined(Pr2)
	profile_combined(Pr3)
	profile_combined(Pr4)
	profile_combined(Pr5)
	
	profile_combined(Prs1)
	profile_combined(Prs2)
	profile_combined(Prs3)
	profile_combined(Prs4)
...
function transformProfile(Pr)
    ...
    c1s = (cx + 1000Pr.R₁ * cosd(90  +  Pr.ϕ₁ᵗ            ),
		   cy + 1000Pr.R₁ * sind(90  +  Pr.ϕ₁ᵗ            ))
    c1e = (cx - 1000Pr.R₁ * cosd(90  +  Pr.ϕ₁ᵇ            ),
		   cy - 1000Pr.R₁ * sind(90  +  Pr.ϕ₁ᵇ            ))
	c1m = (cx + 1000Pr.R₁ * cosd(180 + (Pr.ϕ₁ᵗ + Pr.ϕ₁ᵇ)/2),
		   cy + 1000Pr.R₁ * sind(180 + (Pr.ϕ₁ᵗ + Pr.ϕ₁ᵇ)/2))
    
	c2s = (cx + 1000Pr.l + 1000Pr.R₂ * cosd(90 + Pr.ϕ₂ᵗ               ),
		   cy + 1000Pr.ξ + 1000Pr.R₂ * sind(90 + Pr.ϕ₂ᵗ               ))
	c2e = (cx + 1000Pr.l - 1000Pr.R₂ * cosd(90 + Pr.ϕ₂ᵇ               ),
		   cy + 1000Pr.ξ - 1000Pr.R₂ * sind(90 + Pr.ϕ₂ᵇ               ))
	c2m = (cx + 1000Pr.l + 1000Pr.R₂ * cosd(    (Pr.ϕ₂ᵗ + Pr.ϕ₂ᵇ) / 2 ),
		   cy + 1000Pr.ξ + 1000Pr.R₂ * sind(    (Pr.ϕ₂ᵗ + Pr.ϕ₂ᵇ) / 2 ))
    
    pls = (cx + 1000Pr.xt[1]  , cy + 1000Pr.yt[1]  )
    ple = (cx + 1000Pr.xt[end], cy + 1000Pr.yt[end])
    pl1 = (cx + 1000Pr.xt[1]   +                (Pr.xt[end] - Pr.xt[1]) * 1000/3,
		   cy + 1000Pr.yt[1]   + tand(Pr.ϕ₁ᵗ) * (Pr.xt[end] - Pr.xt[1]) * 1000/3)
    pl2 = (cx + 1000Pr.xt[end] -                (Pr.xt[end] - Pr.xt[1]) * 1000/3,
		   cy + 1000Pr.yt[end] - tand(Pr.ϕ₂ᵗ) * (Pr.xt[end] - Pr.xt[1]) * 1000/3)
    
    sls = (cx + 1000Pr.xb[1]  , cy + 1000Pr.yb[1]  )
    sle = (cx + 1000Pr.xb[end], cy + 1000Pr.yb[end])
	sl1 = (cx + 1000Pr.xb[1]   +                (Pr.xb[end] - Pr.xb[1]) * 1000/3,
		   cy + 1000Pr.yb[1]   + tand(Pr.ϕ₁ᵇ) * (Pr.xb[end] - Pr.xb[1]) * 1000/3)
    sl2 = (cx + 1000Pr.xb[end] -                (Pr.xb[end] - Pr.xb[1]) * 1000/3,
		   cy + 1000Pr.yb[end] - tand(Pr.ϕ₂ᵇ) * (Pr.xb[end] - Pr.xb[1]) * 1000/3)
...
begin
	...
	md"Запись профилей рабочих лопаток для waterfall-cad"
end

# ╔═╡ 0fb5895e-2d20-4716-86ba-3ee7a3c55433
begin
	write_Haskell(transformProfile.([Prs1, Prs2, Prs3, Prs4, Prs5]), "Data_stator")
	md"Запись профилей сопловых лопаток для waterfall-cad"
end
...
function table_swirl_short()
	...
	| $\alpha^*_2 \degree$    |$(r̂1.α⃰₂)  |$(r̂2.α⃰₂)  |$(r̂3.α⃰₂)  |$(r̂4.α⃰₂)  |$(r̂5.α⃰₂)  |
