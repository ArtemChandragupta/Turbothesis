...
begin
	function distance(xt, yt, xb, yb, Δ)
		d̄ = []
	
		for i in 1:length(xt)
			j  = argmin( √( (xt[i]-xb[j])^2 + (yt[i]-yb[j]-Δ)^2 ) for j in 1:length(xb))
			dᵢ = minimum(√( (xt[i]-xb[j])^2 + (yt[i]-yb[j]-Δ)^2 ) for j in 1:length(xb))
...
# ╔═╡ f19e853e-31dd-47d7-805f-7594e0fd14e0
