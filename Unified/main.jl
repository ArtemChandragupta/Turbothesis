...
begin
	function calc_turb(C, Å, 𝒞 = CONST, 𝒯 = TASK)
		...
		d₂ₘ  = 60 * 𝒞.Y / (π * 𝒯.n) * √(2H₀ₜ / 𝒞.m)
		u₂   = π * d₂ₘ * 𝒯.n / 60
		l₂   = F₂ₜ / (π * d₂ₘ)
		kₘ   = d₂ₘ / l₂
		
		(; T⃰₀, P⃰₀, Nₖ, Gᵧ, Nₜ, Hᵤₜ, ΔT⃰ₜ, T⃰₂ₜ, aᵏʳ₂, c₂ₜ, Hₐₜ, H₀ₜ, T⃰₂ₜₜ, P₂ₜ, T₂T, ρ₂ₜ, F₂ₜ, d₂ₘ, u₂, l₂, kₘ)
	end
...
begin
	function stage_params(T, Φ, Ψ, l̄, 𝒞 = CONST, 𝒯 = TASK)
		...
		l₂₁ = Tuple(l̄.ll₂[1:2:end]) # Высота передней кромки сл
		b₂  = Tuple(l̄.xl₂[i] - l̄.xl₂[i-1] for i in 2:2:length(l̄.xl₂)) # Ширина рл
