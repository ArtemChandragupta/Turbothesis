...
На @orig[рисунке] изображена блок-схема оригинальной методики вычислений из пособия Кирилла Леонидовича Лапшина для проведения первичного расчета и расчета турбины по ступеням, _а также специальный для расчета закрутки потока в последней ступени турбины по обратному закону, выданный группе 3231303/21201 п/г 2_.
...
#figure(
  [
    #image("assets/G.svg", width: 86%)
    #text(size:12pt)[Красной линией обозначен целевой расход, синей линией обозначена целевая мощность, красной точкой обозначено оптимальное значение параметров]
  ],
  caption: [Зависимость оптимального расхода воздуха через компрессор от параметров $Phi$ и $Psi$.]
) <G>
...
На @Pih[рисунке] представлено такое поле, построенное для среднеквадратичного отклонения ($sigma$) профиля роста давления по радиусу от их усреднённого линейного тренда и разницы суммарной полиномиальной и кинематической степени реактивности.

Минимальным значениям среднеквадратического отклонения $sigma$ и разницы полиномиальной и расчетной степени реактивности $Delta rho_к$ соответсвуют наиболее тёмные области диаграммы Все из них подходят, чтобы выбрать одну и обеспечить сохранение декларативности, выбирается точка с наименьшим градиентом, потому что в случае ошибки моделирования или изготовления влияние этой ошибки на анализируемый параметр будет также минимизировано.

#v(-2em)

#figure(
  image("assets/var.svg", width:100%),
  caption: [Диаграммы зависимости $sigma$ и $Delta rho_k$ от $F$ и $rho_k$ при заданных $alpha_1$ и $beta_2^*$]
) <Pih>

На @good[рисунке] показаны графики роста кинематической и термодинамической степеней реактивности и давления на выходе из последней ступени турбины.

#figure(
  image("assets/goodies.svg", width:100%),
  caption: [Зависимости $sigma$ и $Delta rho_k$ от $F$ и $rho_k$ при заданных $alpha_1$ и $beta_2^*$]
) <good>
