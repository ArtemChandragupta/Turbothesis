...
begin
	const TASK = (
		T₃ = 1643.15,  # Температура газа перед турбиной
		...
		πₖ = 18,       # Степень повышения давления
	)
	...
	const CONST = (
		...
		σ₁   = 0.987, # Коэффициент потерь на входном устройстве
		σₖₛ  = 0.96,  # Коэффициент потерь в камере сгорания
		σ₄   = 0.985, # Коэффициент потерь в выходном устройстве
		ηₐ   = 0.91,  # Адиабатный КПД компрессора
		ηₚ   = 0.87,  # Политропный КПД турбины
		ηₘₜ  = 0.995, #
		ηₘₖ  = 0.995, #
		ηₖₛ  = 0.98,  #
	)
...
function calc_prime(𝒞 = CONST, 𝒯 = TASK)
	...
	P₂  = 𝒯.πₖ * P₁
	T₂  = 𝒯.Tₙ * 𝒯.πₖ^((kₙ-1)/kₙ)
	Hₒₖ = 𝒞.Cpₙ * T₁ * ( 𝒯.πₖ^((kₙ-1)/kₙ) - 1 )
	Hₖ  = Hₒₖ / 𝒞.ηₐ
	P₃  = P₂ * 𝒞.σₖₛ
	P₄  = 𝒯.Pₙ / 𝒞.σ₄
	πₜ  = P₃ / P₄
	Hₒₜ = 𝒞.Cpᵧ * 𝒯.T₃ * (1 - πₜ^(-𝒞.k_1k))
	Hₜ  = Hₒₜ * 𝒞.ηₚ
	T₄  = 𝒯.T₃ * πₜ^(-𝒞.k_1k)
	...
	(; kₙ, P₁, T₁, P₂, T₂, Hₒₖ,	Hₖ,	P₃, P₄,	πₜ,	Hₒₜ, Hₜ, T₄, Gₙ, Q̇₁, Q₁, ηe, Φ)
end
...
function calc_coolant(I, 𝒞 = CONST, 𝒯 = TASK)
	...
	t₂   = I.T₂ - T̂₀
	t₃   = 𝒯.T₃ - T̂₀
	gₐᵢᵣ = (Qₙₚ*𝒞.ηₖₛ + hₜₒₚ + L₀*𝒞.Cpₙ*t₂ - (L₀+1)*Cpₐ*t₃) / (Cpₐ * (t₃-t₂))
	a    = (L₀ + gₐᵢᵣ)/ L₀
	gₜ   = 1 / (a * L₀)
	gᶜc  = 0.01 + 0.25 / 10000 * (𝒯.T₃ - Tₛₜ)
	gᵖc  = 0.08 + 0.22 / 10000 * (𝒯.T₃ - Tₛₜ)
	gc   = σᵤₜ * (gᶜc + gᵖc)
	...
	(; Qₙₚ, hₜₒₚ, L₀, T̂₀, Cpₐ, Tₛₜ, σᵤₜ, t₂, t₃, gₐᵢᵣ, a, gₜ, gᶜc, gᵖc, gc, ĝc, Gₜ, Ωᵣₐₛ, Hₑ, Ωₐₗₗ,)
end

# ╔═╡ fe821429-f573-4fb8-9268-54aeb6be6e49
function calc_comp(I, C, πₖ, 𝒞 = CONST, 𝒯 = TASK)
	σᵢₙ  = 0.99
	σₒᵤₜ = 0.98
	ηₐ   = 0.88
	nₖ   = 1.463
	...
	l₂   = 1/2 * (1/V₂ - 1) * Dᵥₜ₁
	uₙ₁  = (𝒯.n * π * D₁) / 60
	Hₐ   = (I.kₙ / (I.kₙ-1)) * 𝒞.Rₙ * T₁ * ( (P₂/P₁)^((I.kₙ-1)/I.kₙ) - 1 )
	Hₖ   = Hₐ / ηₐ
	i    = ceil(Hₖ / hₘ)
	h₁   = 0.65 * hₘ
	h₂   = 1.15 * hₘ
	...
	c₁   = √(cᶻ₁^2 + cᵤ₁^2)
	α₁   = atand(cᶻ₁ / cᵤ₁)
	T₁¹  = T₁ - c₁^2 / ( 2 * 𝒞.Rₙ * 1000 * (I.kₙ / (I.kₙ-1)) )
	wᵤ₁  = cᵤ₁ - u
	w₁   = √(cᶻ₁^2 + wᵤ₁^2)
	Mʷ₁  = w₁ / (I.kₙ * 1000 * 𝒞.Rₙ * T₁)^2
	β₁   = atand(cᶻ₁ / (-wᵤ₁))
	Δcᶻ  = (cᶻ₁ - cᶻ₂) / i
	Ocᶻ₂ = cᶻ₁ - Δcᶻ/2
	c₂   = √(Ocᶻ₂^2 + cᵤ₂^2)
	...
	ϵ    = β₂ - β₁
	Φₙ   = cᶻ₁ / uₙ₁
	Mʷₘ  = uₙ₁ * √(1 + Φₙ^2) / √(I.kₙ * 1000 * 𝒞.Rₙ * T₁)
