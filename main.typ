...
#figure(
  ...
  caption: [Изображение профиля окружной скорости за рабочими лопатками последней ступени по радиальному направлению]
) <spec-sector>
...
#noind где $p_2, rho_2, c_2$ --- значения давления, плотности и скорости потока при входе в диффузор; $p_"out"$ --- среднее значение давления на выходе из диффузора.
...
Исследования были проведены на модели "бандажированная турбинная ступень --- осевой диффузор" газовой турбины мощностью 1000 МВт, выполненной в масштабе 1:8.53 и установленной на экспериментальном стенде ЭТ-4 в лаборатории Турбиностроения. Эскиз стенда показан на @spec-model.

#figure(
  {
    image("assets/spec/model.png")
    text(12pt)[
      1 --- направляющий аппарат; 2 --- рабочие бандажированные лопатки; 3 --- силовые стойки; 4 --- кольцевой диффузор; 5 --- конический диффузор; 6 --- хонейкомб; 7 --- гидротормоз; 8 --- ротор; 9 --- дистанционное кольцо. 
    ]
  },
...
#figure(
  {
    lq.diagram(
      legend: (inset:10pt, position: right),
      width: 15cm,  height: 9cm,
      ylim: (0, 1), xlim: (0.88, 1.1),
      xaxis: (subticks: 1),
      yaxis: (subticks: none),
      xlabel: text(12pt)[Относительная длина диффузора], ylabel: $C_p$,
      grid: (stroke-sub: 0.1pt),
    ...
    text(12pt)[Сплошной линией показано статическое давление, пунктирной --- полное]
  },
