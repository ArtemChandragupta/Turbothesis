...
= Выбор оптимальных параметров цикла <chapter-A2GTP>
...
Осевая составляющая скорости на выходе из РК первой ступени:
...
Важным параметром современной камеры сгорания энергетической ГТУ является её экологичность --- ГТУ с высокоэкологичной камерой сгорания можно эксплуатировать в городской застройке. Основными загрязняющими веществами для ГТУ являются:
+ Оксиды азота.
+ Оксиды углерода.
...
= Профилирование меридианных обводов проточной части <section-mer>
...
= Расчет турбины по среднему диаметру <Chapter-S>
...
#figure(
  {
    image("assets/plots/G.svg", width: 85%)
...
На @Block-1 показана условная схема алгоритма расчета. Под исходными данными понимаются данные всех предыдущих расчетов. Предварительный расчет турбины во время варьирования выполняется по формулам, показанным в @chapter-turb, расчет турбины по ступеням --- по формулам, показанным в @S. В схеме используются обозначения:
- $[R], [R^*]$: массив всех параметров и результатов варьирования и массив параметров и результатов, претендующих на соответствие искомомым;
- $R^*$: результат, обеспечивающий совпадение расхода и мощности;
- $G_i, G_A$: расход газа через турбину на этапе варьирования и полученный в @chapter-A2GTP;
- $G_(#sym.Phi, Psi)^"opt", H_(#sym.Phi, Psi)^"opt", (alpha_2)_(#sym.Phi, Psi)^"opt" $: значения расхода газа, перепада энтальпий на рабочих колёсах и угла выхода потока из последней ступени, при которых этот угол наиболее близок к прямому при конкретных $#sym.Phi, Psi$;
- $Delta N_i, Delta N_min$: разница номинальной мощности и мощности при варьировании и минимальное такое значение соответственно.
...
#figure(
  text(8pt, diagram(
    ...
    node(name: <InitDelta>, (-1,4),
      [Инициализация: \ $(alpha_2)_(#sym.Phi, Psi)^"opt" = infinity, G_(#sym.Phi, Psi)^"opt", H_(#sym.Phi, Psi)^"opt" $]
    ),
    edge("-|>"),
    ...
    node((-1,6), [Предварительный \ расчет турбины с $G_i$]),
    edge("-|>"),

    node((-1,7), [Расчет турбины \ по ступеням]),
    edge("-|>"),

    node(name:<IfG>,(-1,8), shape: diamond,
      $ abs((alpha_2)_i - 90 degree) < abs((alpha_2)_(#sym.Phi, Psi)^"opt" - 90 degree ) $
    ),
    edge("-|>", [Да]),

    node(name: <Gopt>, (-1, 9),
      $G_(#sym.Phi, Psi)^"opt" = G_i, H_(#sym.Phi, Psi)^"opt" = H_i, (alpha_2)_(#sym.Phi, Psi)^"opt" = (alpha_2)_i $
    ),
    edge(<Gopt.east>, (-0.32,9), "u,u,u,u" ,<CycG.east>, "-|>"),
    edge(<IfG.east>, (-0.36,8), "u,u,u", [Нет], label-side: left, label-pos: 30% ),

    node(name: <G>, (-1,10), shape: diamond,
      $ abs(G_(#sym.Phi, Psi)^"opt" - G_A) < 0.1 $
    ),
    edge("-|>", [Да]),

    node((-1,11),[Запись в $[R^*]$ ]),
    edge("-|>"),

    node(name: <result>, (-1,12), [Запись в $[R]$ ]),

    edge(<result.west>, "l,u,u,u,u,u,u,u,u,u", <Cyc2.west>, "-|>"),
    ...
    node((1,5), $Delta N_i = abs( H_i dot G_A / k_(n t) - N_t)$),
    edge("-|>"),

    node(name: <IfN>, (1,6), shape: diamond,
      $ Delta N_i < Delta N_min $
    ),
    edge("-|>", [Да]),
    
    node(name:<DN>,(1,7),[$Delta N_min = Delta N_i$, $R^* = R^*_i$]),
    edge(<DN.east>, "r,u,u,u", <CycRes.east>, "-|>"),
    edge(<IfN>,"r,u,u", [Нет], label-side: left, label-pos: 35%),

    node(name: <end-start>, (0,5), [Расчет турбины \ по ступеням \ по данным $R^*$]),
    edge("-|>"),

    node((0,6),[Построение графика \ по данным $[R]$ \ для наглядности]),
    edge("-|>"),
...
#let tbl_S = table(
  ...
  $ p_2^* = p_2 dot (frac( T_2^*, T_2, style: "horizontal"))^((k-1)/k) $, [Па],
  ...
  $ T^*_(2 t t) = T_(2 t t) dot (frac(p_2^*, p_2, style: "horizontal"))^((k-1)/k) $, [К],
...
#figure(
  lq.diagram(
    height: 8cm, xaxis: (tick-distance: 1, subticks: none),
...
В расчете происходит варьирование по четырем параметрам:
+ Угол потока на входе в рабочую лопатку в абсолютном движении на периферии: $ 13 degree < alpha''_1 < alpha_(1 "ср")$.
+ Угол потока на выходе из рабочей лопатки в относительном движении у корня: $15 degree < beta_2^* ' < 65 degree$.
...
#figure(
  ...
  caption: [Поле распределения значений $sigma$ и $Delta rho$ при допустимых значениях параметров],
) <var>
...
$ b_1 = r_(1 "пер")^(n_1) dot tg(alpha_(1 "пер")) = R5r ^ SIn1 dot tg( R5α1 degree ) = SIb1. $ <Swirl-2>

Значения констант для расчета закрутки потока на выходной кромке:

$ n_2 = (ln tg(beta_(2 "пер")^*)/ tg(beta_(2 "ср")^*) )/ ln(r_(2 "пер") / r_(2 "ср")) = (ln ( tg(R5βs2 degree )/ tg(R3βs2 degree) ) )/ ln( R5r/R3r ) = SIn2, $ <Swirl-3>

$ b_2 = r_(2 "ср")^(n_2) dot tg(beta_(2 "ср")^*) = R3r^SIn2 dot tg(S4βs2 degree) = SIb2. $ <Swirl-4>

Коэффициенты для определения осевой составляющей скорости в абсолютном движении на выходе из соплового аппарата:

$ A = (F dot c_(1 z "ср"))/(r_(1 "пер")-r_(1 "ср")) = (SIF dot R3c1z ) / ( 0.5 dot P4l1 ) = SIA, $ <Swirl-5>

$
  B &= c_(1 z "ср") - (F dot c_(1 z "ср"))/(r_(1 "пер")-r_(1 "ср")) dot r_(1 "ср") = \ &= R3c1z - (SIF dot R3c1z)/( 0.5 dot P4l1) dot R3r = SIB.
$ <Swirl-stop>

На @Block-2 изображена условная схема алгоритма расчета. Под исходными данными понимаются данные всех предыдущих расчетов, а также, а также заранее назначенные углы конусности и диаметры входной и выходной кромки в корневом сечении РЛ. Под расчетом коэффициентов закрутки потока понимается расчет параметров по формулам @Swirl-start[]-@Swirl-stop[]. Расчет закрутки потока осуществляется по формулам, приведённым в @R, причём:

+ Первым выполняется расчет для среднего сечения, затем --- по возрастанию от корневого до периферийного. Такой порядок требуется по причине того, что расчет параметров всех сечений опираются на параметры среднего сечения.

+ Для среднего сечения параметры берутся из расчета по среднему диаметру, представленные в @Chapter-S.

+ Окружная составляющая скорости в относительном движении на корневом диаметре на выходе из рабочего колесa:
  $
    w'_(2 u) 
...
На схеме употребляются следующие обозначения:
- $[R]$: массив всех параметров варьирования, удовлетворяющих критериям монотонного роста давлений за РЛ, кинематической и термодинамической степеней реактивности по сечениям, величины модуля суммы разности рассчетной и полиномиальной степени реактивности по всем сечениям, а также конфузорности канала РК в корневом сечении.
- $R^*$: оптимальное значение параметров варьирования;
- $sigma_min$: минимальное значение параметра $sigma$, соответствующее оптимальному значению.

Для проверки конфузорности канала РК в корневом сечении по методу, подробно описанному позже в @Chapter-hs, строится профиль РЛ в этом сечении, по его параметрам вычисляется оптимальный шаг, по которому строится решётка из двух профилей. Для упрощения расчета, конфузорность на данном этапе оценивается по четырём точкам корытца, более подробные выводы о конфузорности межлопаточных каналов РК и СА получены в @Chapter-hs.
...
#figure(
  text(8pt, diagram(
    ...
    node(name: <P0>, (0,-1), corner-radius: 10pt, [
      Получение исходных данных
    ]), edge("-|>"),

    node(name: <Pa>, (0,0), [
      Вводятся $alpha''_1, beta_2^* ' $ 
    ]), edge("-|>"),
    ...
    node((-1,4),[Расчет коэффициентов \ закрутки потока]),
    edge("-|>"),

    node((-1,5),[Расчет закрутки потока]),
    edge("-|>"),

    node((-1,6),[Расчет полиномиальной \ степени реактивности, \ а также $Delta rho = abs(Delta rho_K)$ ]),
    edge("-|>"),

    node(name: <IfK>, (-1,7), shape: diamond, inset: 0.7em, [
      Проверка критериев: \
      1) Монотонный рост $p_2, rho_T, rho_K$ \
      2) Конфузорность корневого канала \
      3) $Delta rho < 0.1$
    ]),
    edge("-|>", [Да]),

    node((-1,8), [Расчет $sigma$]),
    edge("-|>"),

    node(name: <ResS>, (-1,9), [Запись в $[R]$]),
    edge(<ResS.west>, (-3.5,9), "u,u,u,u,u,u", <Cyc2.west>, "-|>"),
    edge(<IfK>, (-3,7), (-3,3), [Нет], label-pos: 10%),
    edge(<Cyc1.east>, (1,2), <LeftS.north>, "-|>", [Конец цикла], label-pos: 20%),
    ...
    node(name: <ResL>, (1,6), $R^* = R_i, sigma_min = sigma_i$),
    edge(<ResL.east>, (2.5,6), "u,u",<CycL.east>, "-|>"),
    ...
    node((0,7), [Построение графика \ по данным $[R]$ и $R^*$ \ для наглядности]),
    edge("-|>"),

    node((0,8), corner-radius: 10pt, [
      Вывод всех результатов \ и их передача \ в следующие расчеты
    ]),
  )),
  caption: [Условная схема алгоритма расчета закрутки потока]
) <Block-2>

#pagebreak()

Результаты расчета закрутки на последней ступени по "обратному" закону для пяти сечений по найденным оптимальным параметрам представлены в @R[таблице].
...
= Построение моделей рабочей и сопловой лопаток <Chapter-hs>

Основываясь на углах из @R[таблицы], строятся треугольники скоростей и профили для 5 сечений рабочей и сопловой лопатки последней ступени турбины. Построение профилей проводится по средней линии и использует кубический сплайн в форме Эрмита для описания средней линии, спинки и корытца @KOR. Такой подход позволяет автоматизировать процесс, что выгодно отличает его от метода окружностей, а также упрощает компьютерную обработку и анализ полученных профилей.
...
Построение описанной кривой изображено на @hermite-spline.

#figure(
  cetz.canvas({
    import cetz.draw: *
    let (a, b, c, d) = ((0, 2), (10, 0), (4, -2), (8, -3))
    bezier(name: "line", a, b, c, d, stroke: 2pt)

    line(name: "ar", a, c, mark: (end:"stealth", fill:red), stroke: red)
    line(name: "br", b, ( 2*b.at(0) - d.at(0), 2*b.at(1) - d.at(1) ), mark: (end:"stealth", fill:blue), stroke: blue)

    line(name:"ax",a,(rel:(x:2)), mark: (end:"stealth",fill:black))
    line(name:"bx",b,(rel:(x:2)), mark: (end:"stealth",fill:black))

    cetz.angle.angle("ar.start", "ax.end", "ar.end", label:$alpha_1$,label-radius: 150%, radius: 50%, direction: "cw", mark: (symbol:"stealth", fill:black))
    cetz.angle.angle("br.start", "bx.end", "br.end", label:$alpha_2$,label-radius: 150%, radius: 50%, direction: "cw", mark: (symbol:"stealth", fill:black))

    content(a,$(x_1,y_1)$,anchor:"south",     padding: 0.2)
    content(b,$(x_2,y_2)$,anchor:"south-west",padding: (y:0.2,x:0.4))
    circle(a, radius: 2pt, fill: black)
    circle(b, radius: 2pt, fill: black)

    line(name: "xl", (-2,-3), (14, -3), mark: (end:"stealth",fill:black))
    line(name: "yl", "xl.start", (rel:(y:7)), mark: (end:"stealth",fill:black))
    content("yl.end", [y], anchor: "east", padding:0.1)
    content("xl.end", [x], anchor: "north", padding:0.1)

    line(name:"x1",a, (y:-3),            stroke: (dash:"dashed"))
    line(name:"x2",b, (y:-3, x:b.at(0)), stroke: (dash:"dashed"))

    cetz.decorations.brace(name: "br", "x1.end","x2.end", flip: true)
    content("br.spike", [h], anchor: "north", padding: 0.1)

    line(name: "xx", "line.60%", (horizontal:(), vertical:"xl"), stroke: (dash:"dashed"))
    circle("xx.start", radius: 2pt, fill: black)
    content("xx.start",$(x, p(t))$, anchor: "south-west", padding: 0.2)
  }),
  caption: [Сплайн в форме Эрмита]
) <hermite-spline>
...
Построения профиля лопатки этим методом происходит по следующим шагам (для рабочей лопатки берется скорость в относительных координатах, для сопловой --- в абсолютных):
+ Строится треугольник скоростей для рассматриваемого сечения.
+ По длине меридианного сечения лопатки, полученной в @section-mer, а также по среднему вектору скорости строится хорда лопатки.
+ С центрами на краях хорды строятся окружности входной и выходной кромок.
+ По начальной и конечной точкам хорды, а также углам входной и выходной скорости, с помощью сплайна в форме Эрмита строится средняя линия лопатки.
+ По углам в начале и конце средней линии и углам конусности у входной и выходной кромок находятся начальный и конечный углы линий спинки и корытца профиля, по которым также находятся касательные точки к окружностям входной и выходной кромок. По полученным точкам и углам строятся линии спинки и корытца.

Пример полученного таким образом профиля показан на @test-profile.

#figure(
  {
    image("assets/profiles/profile_spline.png", width: 60%)
    text(12pt)[Красным цветом показана линия корытца, синим цветом показана линия спинки]
  },
  caption: [Профиль рабочей лопатки, построенный с помощью сплайнов в форме Эрмита]
) <test-profile>

По построенным профилям определяется число лопаток на РК и в СЛ.
...
#noind где $c$ --- максимальная толщина профиля, $b$ --- длина хорды профиля.
...
На рисунках
// @Tri-1[]-@blade-last[]
показаны треугольники скоростей и решетки профилей соответствующих сечений рабочих лопаток. Для демонстрации конфузорности каналов для каждой точки корытца первого профиля были подобраны ближайшие точки спинки второго, по этим данным построено поле распределения расстояния между профилями вдоль канала. Расстояния в этом поле показаны с помощью перцептивно равномерной цветовой шкалы viridis @Viridis, которая позволяет однозначно сопоставить цвет со значением.
...
#figure(
  image("assets/profiles/2/profile_combined1.svg", width:62%),
  caption: [Профили РЛ в корневом сечении и демонстрация конфузорности канала между ними]
)
...
#figure(
  image("assets/profiles/2/profile_combined2.svg", width:62%),
  caption: [Профили РЛ в средне-корневом сечении и демонстрация конфузорности канала между ними]
)
...
#figure(
  image("assets/profiles/2/profile_combined3.svg", width:62%),
  caption: [Профили РЛ в среднем сечении и демонстрация конфузорности канала между ними]
)
...
#figure(
  image("assets/profiles/2/profile_combined4.svg", width:62%),
  caption: [Профили РЛ в средне-периферийном сечении и демонстрация конфузорности канала между ними]
)
...
На рисунках @Tris-1[]-@blade-s-last[] изображены треугольники скоростей и решетки профилей соответствующих сечений сопловых лопаток.
...
#figure(
  image("assets/profiles/1/profile_combined1.svg", width:62%),
  caption: [Профили СЛ в корневом сечении и демонстрация конфузорности канала между ними]
)
...
#figure(
  image("assets/profiles/1/profile_combined2.svg", width:62%),
  caption: [Профили СЛ в средне-корневом сечении и демонстрация конфузорности канала между ними]
)
...
#figure(
  image("assets/profiles/1/profile_combined3.svg", width:62%),
  caption: [Профили СЛ в среднем сечении и демонстрация конфузорности канала между ними]
)
...
#figure(
  image("assets/profiles/1/profile_combined4.svg", width: 62%),
  caption: [Профили СЛ в средне-периферийном сечении и демонстрация конфузорности канала между ними]
)
...
#figure(
  image("assets/profiles/1/profile_combined5.svg", width: 62%),
  caption: [Профиль СЛ в периферийном сечении и демонстрация конфузорности канала между ними]
) <blade-s-last>
...
+ $alpha_1 = 80 degree$ --- угол входа в сопловой аппарат последней ступени.
...
Для проведения расчета в блоке Static Structural создаётся циклограмма нагружения, моделирующая условия эксплуатации турбины. Для учёта упрочнения, циклограмма представляет из себя два последовательных цикла пуска и останова, значения для оценки прочности лопатки берутся из второго цикла. Используемая циклограмма показана на @cycle-blade.
...
#figure(
  lq.diagram(
    ...
    lq.line((2*ome  ,0  ),(3*ome, trg), stroke: blue + 1pt),
    lq.line((3*ome,trg),(4*ome, 0), stroke: blue + 1pt),
  ),
...
Практическая реализация расчета размаха деформаций в Ansys производится с помощью функции Used Defined Result. На @user-blade изображена построенная с помощью этой функции эпюра распределения размаха деформаций.
...
#figure(
  image("assets/ansys/disk/boundary_conditions.png", width:100%),
  caption: [Граничные условия для сектора диска]
) <disk-gra>
...
#figure(
  image("assets/ansys/disk/elastic/stress.png", width:100%),
  caption: [Распределение эквивалентных напряжений]
) <disk-u-str>
...
#figure(
  image("assets/ansys/disk/elastic/deformation.png", width:100%),
  caption: [Распределение абсолютных деформаций]
) <disk-u-def>
...
#figure(
  lq.diagram(
    ...
    lq.yaxis( exponent: 0, position: right, label: $F, Н$,
      ...
      lq.line((3 * ome, 90.5), (4 * ome, 0), stroke: blue + 1pt),
    ),
  ),
  caption: [Циклограмма нагружения диска рабочего колеса]
) <disk-cycl>

Для отображения истории сходимости используется параметр Force Convergence (@disk-res[рисунок]).
...
По результатам проведения расчета построены эпюры распределения эквивалентных напряжений (@disk-p-str[рисунок]) и эквивалентных пластических деформаций (@disk-p-pla[рисунок]).

#figure(
  image("assets/ansys/disk/plastic/stress.png", width:100%),
  caption: [Эпюра распределения эквивалентных напряжений]
) <disk-p-str>
...
#figure(
  image("assets/ansys/disk/plastic/deformation_plastic.png", width:100%),
  caption: [Эпюра распределения эквивалентных пластических деформаций]
) <disk-p-pla>

Как видно из эпюр, эквивалентные напряжения по фон-Мизесу в упруго-пластической постановке показывают снижение по сравнению с упругим расчетом с #zMPa(7851.6) до #zMPa(139.33). Это объясняется перераспределением внутренних сил в рамках больших площадей, возникающих в результате пластической деформации материала. О наличии этих деформаций свидетельствует ненулевое значение эквивалентных пластических деформаций.

Далее проводится оценка ресурса диска при симметричном циклическом нагружении в условиях пластического деформирования. Для этого необходимо рассчитать количество циклов до разрушения диска.

Аналогично расчету количества циклов до разрушения рабочей лопатки в @Ansys-blade-up, с помощью пользовательских функций считается размах деформаций по диску, на основе которого определяется критическая точка с максимальным значением $Delta epsilon $ (@disk-de[рисунок]).

#figure(
  image("assets/ansys/disk/plastic/de.png", width:100%),
...
В результате расчета было получено значение $N = 948$ циклов, оно 
...
Расчет диска на длительную прочность производится аналогично такому расчету для рабочей лопатки в @blade-long. Для стали 26NiCrMoV11-5 уравнение Ларсена-Миллера имеет следующий вид:
...
$ n_"дл.п" = sigma_"дл.п" / sigma_"экв" = 534.6 / 139.33 = 3.84 > 1.5, $
...
В проектируемой установке используется 16-ступенчатый осевой компрессор c постоянным средним диаметром проточной части. Эскиз компрессора показан на @bob-comp.

Входной конфузор компрессора соединен со входным трактом с помощью фланца. Входной конфузор отливается из чугуна с шаровидным графитом, что обусловлено сложной формой детали и небольшой рабочей температурой. В нижней части детали выполнены наплывы для установки опорно-упорного подшипника и внешние лапы, передающие нагрузки от установки на опоры. На внутренние поверхности конфузора наносится специальное органо-силикатное покрытие, препятствующее образованию и облегчающее удаление льда.
...
Передний корпус компрессора также выполняется литым. В качестве материала выбран высокопрочный чугун EN-GJS-400-18U-LT. На переднем корпусе закреплены поворотные направляющие аппараты --- входной направляющий аппарат и НА двух первых ступеней компрессора. Для их привода предусмотрена система рычагов, управляющихся поворотными кольцами. Помимо поворотных НА, на рассматриваемой детали выполнены проточки для НА 3, 4 и 5 ступеней компрессора.

Задний корпус компрессора выполняется сварным из стали 16Mo3, что связано с ростом температур и давления. Все НА, закреплённые на этом корпусе, являются неподвижными, для фиксации обойм внутри корпуса выполнены расточки. В зоне этого корпуса имеются три отбора:
- За 8 ступенью находятся патрубки для отбора в думмис турбины.
- За 10 ступенью расположены камеры для отбора воздуха на охлаждение турбины и противопомпажные клапаны.
- За 13 ступенью происходит отбор охлаждающего воздуха для турбины.

Выходной диффузор компрессора представляет из себя сварно-литую конструкцию из стали, закрепленную на выходном фланце заднего корпуса. Из диффузора предусмотрен отбор воздуха на охлаждение элементов СА первой ступени турбины. С выходной стороны к диффузору крепится разделитель потока.

Направляющие лопатки 3–16 ступеней имеют прямоугольный хвостовик и заводятся в пазы радиально, где фиксируются кольцевыми шпонками с помощью кольцевого зацепа. Направляющие аппараты 3–13 ступеней выполнены из двух половин, что накладывает требование на четное число лопаток. Для обеспечения тепловых перемещений сопловые аппараты 14–16 ступеней поделены на четыре части с целью организации тепловых зазоров в местах сопряжений. Материалы направляющих лопаток: 1–12 --- 20Х13Ш; 13–16 --- 20Х13. За последней ступенью расположен спрямляющий аппарат, устраняющий вихри в потоке.

Рабочие лопатки имеют хвостовик типа "ласточкин хвост" и заводятся в пазы в осевом направлении. Материалы рабочих лопаток компрессора: на 1–2 ступени --- X4CrNiMo16-5-1,  на 3–12 ступени --- 20Х13Ш; 13–16 ступени --- 15Х11МФ-Ш. Фиксация рабочих лопаток 1–6 ступеней осуществляется с помощью стопорных пластин, на остальных ступенях применяется фиксация расчеканиванием металла на торце лопатки. Перо лопаток всех ступеней покрыты противоэрозионно-противокоррозионным покрытием.

Материалы дисков рабочих колёс компрессора: на 1–13-й ступени --- 26NiCrMoV11-5, на 13–16-й ступени --- 26NiCrMoV14-5. Пазы для фиксации лопаточных хвостовиков в дисках изготавливаются с помощью протяжки.
...
Выходной диффузор проектируемой установки имеет угол раскрытия $2 gamma = 13 degree$, что позволяет избежать отрыва потока от стенок.
...
+ Более медленный рост стоимости изготовления при росте нагрузке.
...
Однако, поток на выходе из последней ступени не является однородным и имеет сложную, зависящую от радиуса структуру с ненулевыми окружными составляющими, чем обуславливается необходимость проектировать систему "последняя ступень --- диффузор" совместно, а не по отдельности. Одним из эффектов, учёт которых представляет интерес, является течение рабочего тела из зазора над рабочей лопаткой последней ступени у стенки диффузора. Поток в этом зазоре, с одной стороны, уменьшает КПД последней ступени турбины за счет концевых потерь, так как не попадает в межлопаточные каналы рабочих лопаток, но в то же время может повысить КПД системы "последняя ступень --- диффузор" за счет поддержания более низкого статического давления на входе в диффузор, а также реализовать большие углы раскрытия диффузора @SpecEng.

== Описание эффекта <spec-2>

Причиной отрыва пограничного слоя в диффузоре является потеря кинетической энергии заторможенных о стенку частиц потока, которые оказываются неспособны преодолеть положительный градиент статического давления. Минуя рабочие лопатки, поток из зазора сохраняет избыточное статическое давление и абсолютную скорость относительно ядра потока.

После прохождения соплового аппарата поток рабочего тела имеет закрутку, которая выражается в наличии окружной составляющей скорости. При прохождении через межлопаточные каналы рабочих лопаток эта составляющая по большей части срабатывается на обеспечение вращения ротора установки, тогда как у части потока, минующей рабочую лопатку через зазор над ней и прилегающей к стенке диффузора сохраняется значительная окружная составляющая @SpecRus. На @spec-swirl изображено описанное течение на стенке диффузора.
...
#figure(
  ...
  caption: [Изображение течения из зазора у стенки диффузора]
) <spec-swirl>

Благодаря окружной составляющей скорости на прошедшую через зазор внешнюю часть потока со со стороны стенки диффузора действует центростремительная сила, которая повышает давление в этом потоке и противодействует его отрыву от стенки, согласно уравнению радиального равновесия:

$ (partial p) / (partial r) = rho c_u^2 / r. $

В результате высокоскоростная струя утечки смешивается с заторможенным пристеночным слоем, передавая ему импульс. Этот эффект смещает точку отрыва потока вниз по течению или полностью ликвидирует отрывную зону, допуская устойчивое безотрывное течение при увеличенных углах раскрытия диффузора $gamma$. На @spec-sector изображено распределение окружной скорости за последней ступенью турбины по радиусу при применении "обратного" закона закрутки с оптимальным радиальным зазором $delta_"opt"$.
...
При повышении угла раскрытия выходного диффузора при его неизменной длине повышается отношение его входной и выходной площади, что теоретически ведёт к росту коэффициента восстановления давления. Эта зависимость видна из формулы для идеального коэффициента восстановления давления:

$ C_p^"ид" = 1 - 1/( frac(A_"out", A_"in", style: "horizontal"))^2, $

#noind где $A_"in"$, $A_"out"$ --- площади живого сечения диффузора на входе и на выходе.

За счет повышения коэффициента восстановления давления выходного диффузора при неизменном давлении на выходе из него $p_"out"$ понижается входное давление $p_2$. Так как это давление является давлением на выходе из последней ступени турбины, его понижение повышает мощность и КПД системы "последняя ступень --- диффузор".

Однако, как отмечалось ранее, та часть потока, которая ответственна за описываемый эффект, минует межлопаточный канал рабочих лопаток, чем вызывает снижение мощности и КПД системы "последняя ступень --- диффузор" и вследствие повышения потерь с выходной скоростью и индуцирования концевых вихрей. Балансом между этими факторами обусловлено существование оптимального зазора, при котором мощность и КПД системы "последняя ступень --- диффузор" будут максимальными.

Этот эффект усиливается применением "обратного" закона закрутки потока при профилировании рабочих лопаток последней ступени, поскольку такой закон обеспечивает перераспределение расходов и градиентов давлений в направлении периферии.
...
#figure(
  ...
  caption: [
    Изменение коэффициента восстановления статического давления по длине диффузора в зависимости от величины радиального зазора по результатам эксперимента и численного моделирования @SpecRus
  ]
) <spec-result>
...
#figure(
  ...
  caption: [Зависимость КПД ступени и КПД системы "последняя ступень --- диффузор" от величины радиального зазора @SpecRus]
) <spec-KPD>
...
#figure(
  ...
  caption: [Распределения относительных полного $dash(p)^*=frac(p^*,hat(p)^*, style: "horizontal")$ и статического $dash(p)=frac(p,hat(p), style: "horizontal")$ давлений по высоте канала на входе в диффузор\ (сечение 2-2) @SpecRus]
) <spec-pressure>
...
+ Выполнен расчет тепловой схемы ГТУ и выбраны оптимальные параметры: $pi_k^* = 16, G_в = #zkg-s(RawAGₙ), G_г = #zkg-s(RawAGᵧ). $
+ Выполнен приближенный расчет 16-ступенчатого компрессора.
+ Выполнен приближенный расчет камеры сгорания.
+ Выполнен аналитический расчет 4-х ступенчатой турбины по среднему сечению.
...
+ Обеспечена и с помощью Ansys Mechanical численно обоснована статическая и вибрационная прочность рабочей лопатки и диска последней ступени турбины: РЛ выдерживает 2174 циклов до разрушения и обеспечивает запас длительной прочности $1.14$, диск выдерживает 948 циклов до разрушения и обеспечивает запас длительной прочности $3.84$.
+ Обеспечена и с помощью Ansys Mechanical численно обоснована вибрационная прочность ротора установки. Отстройка номинального режима от резонанса --- $ 28.6%$.
...
#align(center)[
  = ПРИЛОЖЕНИЕ А. Результаты расчета параметров рабочего процесса ГТУ с помощью A2GTP
]
