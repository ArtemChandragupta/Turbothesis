...
При приближенном расчете осевого компрессора, производимом по методике Ю. С. Подобуева @COMP основными расчетными сечениями являются сечение 1-1 на входе в первую ступень и сечение 2-2 на выходе из последней ступени (@COMP-lop[рис.]). Определим параметры $P$ и $T$ в этих двух сечениях:
...
+ $P_2 = 95649.2$ Па --- статическое давление за последней ступенью.
...
#noind где #context box(baseline: 100% - measure([a]).height, grid(
  ...
  $psi(T,t)$, [--- относительное сужение образца при кратковременном \ разрыве, $D = ln( frac(100,100 - psi,style: "skewed") )$;],
  $sigma_в (T,t)$, [--- предел прочности, МПа;],
  $E(T,t)$, [--- модуль Юнга;],
  $N$, [--- количество циклов.]
))

// #noind где #h(-1.9cm)  #context box(baseline: 100% - measure([a]).height, grid(
//   columns: 2,
//   column-gutter: 0.2em,
//   row-gutter: 0.75em,
//   align: (right, left),
...
#figure(
  ...
  caption: [Циклограмма вращения диска]
) <disk-cycl>
...
Колебания с 2, 3 и 4 узловыми диаметрами являются наиболее критичными для дисков турбин --- именно эти формы колебаний чаще всего возбуждаются в рабочих режимах и могут привести к резонансным явлениям. Вибрационная диаграмма для диска последней ступени турбины представлена на @disk_camp.
...
Исследования были проведены на модели "бандажированная турбинная ступень --- осевой диффузор" газовой турбины мощностью 1000 МВт, выполненной в масштабе 1:8.53 и установленной на экспериментальном стенде ЭТ-4 в лаборатории Турбиностроения. Радиальный зазор на стенде регулируется с помощью сменного дистанционного кольца. Эскиз стенда показан на @spec-model.
