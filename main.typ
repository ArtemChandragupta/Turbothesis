...
Целью данной курсовой работы является создание газовой турбины мощностью 65 МВт на основе ГТЭ-65. В рамках работы будут рассмотрены основные технические характеристики ГТЭ-65, проанализированы её преимущества и недостатки, а также предложены пути оптимизации и улучшения конструкции для достижения заявленной мощности.
...
#figure(
  lq.diagram(
    height:8cm, legend: (position: bottom),
...
#figure(
  lq.diagram(
    height:8cm, ylabel: $phi$, xlabel: $pi_k^*$, cycle: a2gtpmap,
    ..for i in range(csv_data.len()) {
      let Tt = 1443 + 50 * i
...
// #noind где #h(-0.4cm)  #context box(baseline: 100% - measure([a]).height, grid(
//   columns: 2,
//   column-gutter: 0.2em,
//   row-gutter: 0.75em,
//   align: (right, left),
...
#noind где $r$ --- радиус сечения, $alpha$ --- угол потока, $n, b$ --- константы.
...
== Газодинамический турбинной ступени в Ansys CFX <Ansys-GAS>
...
// Выполненный в @Ansys-GAS газодинамический расчет позволяет определить поля температур и давлений для рабочей лопатки в установившемся режиме обтекания. Эти поля используются в качестве исходных данных для прочностного расчета.
...
+ Газодинамическая нагрузка, обусловленная перепадом давления на лопатке;
...
#figure(
  image("assets/ansys/blade/elastic/stress.png", width:67%),
...
#figure(
  image("assets/ansys/blade/elastic/deformation.png", width:65%),
...
#figure(
  table(
    columns: (auto, auto, auto, auto, auto, auto, auto),
    [Область РЛ], [Температура, $degree C$], $sigma_0.2^t, "МПа"$, $sigma_max, "Мпа"$, $sigma_min, "МПа"$, $sigma_"экв", "МПа"$, $n_т$,
...
=== Расчет рабочей лопатки на статическую прочность в рамках упруго-пластической постановки с циклограммой нагружения <Ansys-blade-up>
...
В результате были получены эпюры распределения эквивалентных напряжений (@p-str[рисунок]) и пластических деформаций (@p-pla[рисунок]).
...
#noind где точки $A$ и $B$ определяют максимальный размах полной деформации за цикл в рассматриваемом узле конструкции за весь нормальный цикл работы установки.
...
Для представления зависимости предела длительной прочности от времени и температуры используется уравнение Ларсона-Миллера для никелевого суперсплава. В качестве исходных данных принято  $T=921$ K, а требуемый ресурс $t=30000$ часов. В качестве аппроксимирующей зависимости используется уравнение, приведенное в таблице для данного класса сплавов:
...
Для анализа вибрационной прочности лопатки необходимо построить вибрационную диаграмму, где видна зависимость частоты колебаний лопатки от частоты вращения ротора, а линии, выходящие из начала координат, представляют собой зависимости частот возмущающих сил от частоты вращения для гармоник разной  кратности $k$. Абсциссы точек пересечения лучей с кривыми предельных динамических частот определяют границы зон резонансных частот вращения данной ступени.
...
В качестве граничных условий задается температурное поле диска и закрепления. На верхнем ободе диска устанавливается температура, полученная из расчета лопатки в области хвостовика, а на нижней границе задается температура, отличающаяся на 200 градусов. Это создаст температурный градиент по радиусу диска, характерный для реальных условий эксплуатации турбины, таким образом термические напряжения будут учтены. Полученное распределение температур представлено на @disk-temp.
...
После описанного этапа задаются механические нагрузки. Скорость вращения устанавливается с помощью Rotational Velocity. Далее моделируется нагрузка от лопаток посредством функции Pressure на верхнем ободе диска, рассчитываемая по формуле:
...
После корректного задания всех нагрузок и граничных условий выполняется расчет в рамках упругой постановки. Для анализа результатов используются Equivalent Stress (von-Mises) и Total Deformation (рисунки @disk-u-str[] и @disk-u-def[] соответственно).
...
Проанализировав полученные эпюры можно сделать вывод, что перемещения соответствуют реальности, а значит граничные условия заданы верно. На эпюре эквивалентных напряжений максимальное значение 7851 МПа, что нереалистично, условие прочности при таком значении выполняться не будет и на основании выводов, сделанных в @Ansys-blade-u для рабочей лопатки, для диска также необходимо провести расчет в упруго-пластической постановке.

=== Расчет диска на статическую прочность в рамках упруго-пластической постановки с циклограммой нагружения

Перед началом расчета в блоке Static Structural формируется условная циклограмма нагружения, моделирующая реальные эксплуатационные режимы работы турбины (@disk-cycl[рисунок]).
...
Кроме того, задается зависимость давления лопатки на диск от частоты вращения, так как с изменением оборотов согласно циклограмме меняется центробежная нагрузка. Эта зависимость представлена на @disk-lop.
...
Принимая $T=880$ K и $t = 10000$ с, получаем $sigma_"дл.п"=534.6$ МПа.
...
Для проведения модального расчета по продольному разрезу проектируемой установки построена упрощённая модель ротора, изображенная на @rotor1.
...
По полученной модели твердотельной модели построена сеточная модель, изображенная на @rotor2.
...
#figure(
  {
    lq.diagram(
      ...
      height: 6cm, xaxis: (subticks: 1), yaxis: (subticks: 0),
...
В ходе выполнения выпускной квалификационной работы была спроектирована газотурбинная установка мощностью 65 МВт.
