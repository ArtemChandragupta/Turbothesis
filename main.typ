...
+ Теоретическое удельное количество воздуха
...
Для выполнения расчета в блоке CFX-Pre необходимо задать граничные условия. Указывается число лопаток, 59 рабочих и 52 сопловых. Для сопловой лопатки указывается тип закрепления Stationary (неподвижный), а для рабочих --- Rotating (вращающийся) с частотой вращения $5441 frac("об", "мин", style: "horizontal")$.
...
Между областью сопловой и рабочей лопаток выбран тип интерфейса Stage. Он осредняет параметры потока по окружности, сглаживая нестационарное взаимодействие между решетками. Расчетная модель показана на @InOut.
...
Принимая $T=921$ K и $t=30000$ ч, получаем, что $sigma_"дл.п"=940$ МПа.
...
На верхнем ободе диска устанавливается температура, полученная из расчета РЛ в области хвостовика, а на нижней границе задается температура, отличающаяся на $200 degree C$. Это позволяет построить характерный для условий эксплуатации турбины температурный градиент и учесть термические напряжения. Полученное распределение температур представлено на @disk-temp.
...
По полученным эпюрам можно сделать вывод, что перемещения соответствуют ожиданию, а значит граничные условия заданы верно. Максимальное значение эквивалентных напряжений равно 7851 МПа, что нереалистично, условие прочности при таком значении не выполняется. На основании выводов, сделанных в @Ansys-blade-u для рабочей лопатки, для диска также необходимо провести расчет в упруго-пластической постановке.
...
Как видно из эпюр, эквивалентные напряжения по фон-Мизесу в упруго-пластической постановке показывают снижение по сравнению с упругим расчетом с 7851,6 МПа до 59,61 МПа. Это объясняется перераспределением внутренних сил в рамках больших площадей, возникающих в результате пластической деформации материала.
