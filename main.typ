...
#noind где $pi_k^*$ --- #context box(baseline: 100% - measure([a]).height, [степень повышения давления компрессора (из @opt-cycle[раздела] $pi_k^*=Aπsₖ$).])
...
Камера сгорания проектируемой установки является противоточной и исполняется по трубчато-кольцевому типу. Расчет был выполнен по методике В. А. Рассохина @COMBUSTOR. Продольный разрез КС представлен на @Calc-KS.
...
Газодинамический расчет проводится для качественной оценки обтекания ступени и получения полей температуры и давления на пере РЛ.

== Подготовка модели последней ступени турбины к расчету в ANSYS CFX

Для подготовки к расчету необходимо корректно разместить модели сопловой и рабочей лопаток последней ступени турбины и характерные точки меридионального обвода проточной части. По полученному результату строится FlowPath, необходимый для корректного прохождение моделируемого потока через СА и РК. На @flow-path показана готовая к экспорту в Ansys TurboGrid для построения сетки модель рабочей лопатки.
...
== Газодинамический расчет турбинной ступени в Ansys-CFX <Ansys-GAS>

Для выполнения расчета в блоке CFX-Pre необходимо задать граничные условия. Указывается число лопаток, 59 рабочих и 52 сопловых. Для сопловой лопатки указывается тип закрепления Stationary (неподвижный), а для рабочих --- Rotating (вращающийся) с частотой вращения #zrpm(5441).
...
+ $P_2 = #zPa(95649.2)$ --- статическое давление за последней ступенью.

Между областью сопловой и рабочей лопаток выбран тип интерфейса Stage. Он осредняет параметры потока по окружности, сглаживая нестационарное взаимодействие между решетками.
...
Упругая постановка прочностного расчета проводится в рамках действия закона Гука. Деформации при такой постановке прямо пропорциональны напряжениям, при полном снятии нагрузки тело возвращается в исходное состояние.
...
#figure(
  image("assets/ansys/blade/elastic/stress.png", width:70%),
...
#figure(
  image("assets/ansys/blade/elastic/deformation.png", width:70%),
...
На рисунках @disk_v_2[]-@disk_v_4[] представлена визуализация всех рассчитанных форм колебаний исследуемого диска.

#figure(
  image("assets/ansys/disk/modal/d2.png", width:75%),
  caption:[Веерная форма колебаний с двумя узловыми диаметрами]
) <disk_v_2>

#undo-line()

#figure(
  image("assets/ansys/disk/modal/d3.png", width:75%),
  caption:[Веерная форма колебаний с тремя узловыми диаметрами]
) <disk_v_3>

#undo-line()

#figure(
  image("assets/ansys/disk/modal/d4.png", width:75%),
  caption: [Веерная форма колебаний с четырьмя узловыми диаметрами]
) <disk_v_4>
