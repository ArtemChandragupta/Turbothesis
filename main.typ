...
#figure(
  lq.diagram(
    ...
    lq.scatter((RawTGᵧ,RawTGᵧ),(90,0), size: 12pt, color: blue, label: $G_"опт" = TGᵧ(Phi = P1Φ, Psi = P1Ψ)$),
  ),
  caption: [График зависимости угла потока газа на выходе из среднего сечения последней ступени турбины от расхода газа для конкретных параметров $Phi$ и $Psi$]
) <AnglePlot>

Таким образом, для соответствия расчета турбины по среднему диаметру газодинамическому расчету необходимо провести варьирование этого расчета по параметрам $Phi$ и $Psi$, чтобы найти и назначить такие их значения, при которых достигаются мощность и расход газа $G_"opt"$, равные полученному в @chapter-turb. Результат описанного варьирования показан на @Gopt в виде поля распределения $G_"opt" (Phi, Psi)$, на котором показаны линии целевой мощности и расхода. Видно, что обеспечение необходимых мощности и расхода происходит только в одной точке при параметрах $Phi = P1Φ $, $Psi = P1Ψ $.
...
#figure(
  ...
  caption: [Зависимость оптимального расхода газа через турбину от параметров $Phi$ и $Psi$],
) <Gopt>
...
#figure(
  table(
    ...
    align: horizon,
    [Величина и формула], [Ед.из.], [Ступень 1], [Ступень 2], [Ступень 3], [Ступень 4],
    $ p_0^* = p^*_(2(i-1)) $, [Па],
...
Применение этого закона обеспечивает высокую эффективность выходного диффузора, поскольку повышение давления у периферии "отжимает" поток газа от стенок диффузора, противодействуя центробежной силе, что приводит к более равномерному распределению потока в радиальном направлении. Градиент давления, обеспеченный этим законом закрутки, показан на @p-groth[рисунке].
...
На @rho-groth показано распределение кинематической и термодинамической степени реактивности по сечениям за последней ступенью турбины.
...
Для поиска оптимальных значений этих параметров при ряде выбранных значений $alpha_1$ и $beta_2^*$ было произведено варьирование по параметрам $F$ и $rho_к$, в результате которого было построено поле распределения значений отклонения от линейного роста давлений на выходе из ступени $p_2$,  и разницы суммарной кинематической степени реактивности и суммарной полиномиальной степени реактивности $Delta rho$ при допустимых значениях параметров. Критерием допустимости является монотонный рост давлений $p_2$ от корня к периферии, выраженный в минимизации параметра $sigma$ и значения $Delta rho < 0.1$. Полученные поля изображены на @var.
...
Найдём значения констант для расчета закрутки потока на входной кромке:
...
Найдём значения констант для расчета закрутки потока на выходной кромке:
...
#figure(
  table(
    ...
    $ w_(2 u) = cases( "if"
    i = 1: - (u_1 w_(1u) + 2 u_2^2  rho_k^(i =1))/u_2,
    "else" : w_(2 u)^(i= 1) + ((w_(2u)^(i =3) - w_(2u)^(i =1)) (i-1)) /2  )  $, [м/с],
